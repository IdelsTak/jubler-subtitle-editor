...
public class GoogleJSONTranslator extends SimpleWebTranslator implements Plugin, PluginItem {
  ...
  @Override
  protected String getTranslationURL(String from_language, String to_language)
      throws MalformedURLException {
    return "http://ajax.googleapis.com/ajax/services/language/translate?v=1.0&langpair="
        + findLanguage(from_language)
        + "%7C"
        + findLanguage(to_language);
  }
  ...
  protected String getConvertedSubtitleText(List<SubEntry> subs)
    ...
    for (SubEntry entry : subs) str.append("&q=").append(HTMLTextUtils.encode(entry.getText()));
  ...
  protected String parseResults(List<SubEntry> subs, BufferedReader in) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < subs.size(); i++) {
        ...
        subs.get(i)
            .setText(
                HTMLTextUtils.decode(
                    res.getJSONObject("responseData").getString("translatedText")));
      }
      return null;
    } catch (Exception ex) {
  ...
  public void execPlugin(Object caller, Object params) {
    if (caller instanceof AvailTranslators) ((AvailTranslators) caller).add(this);
  }
