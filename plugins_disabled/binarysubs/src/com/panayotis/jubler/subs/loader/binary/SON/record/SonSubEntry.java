...
import com.panayotis.jubler.subs.loader.SubtitleImageAttribute;
...
/**
 * This class is ued to hold the following parsed information found within the SON subtitle file.
 * This is the subtitle event which shows the event-id, start time, finish time, and the
 * subtitle-image file which holds the subtitle text in a bit-map image. The image should be OCR(ed)
 * to get back the actual editable text. An example of such entry is shown here:
 *
 * <pre>
 ...
 */
public class SonSubEntry extends SubEntry
    implements ImageTypeSubtitle, HeaderedTypeSubtitle, CommonDef {

  public int max_digits = 4;
  public SonHeader header = null;
  public short event_id = 0;
  public SonSubtitleImageAttribute son_attribute = null;
  ...
  public void setMaxImageHeight(int value) {
    if (value > maxImageHeight) maxImageHeight = value;
  }
  ...
  public static short[] makeAttributeEntry(String[] matched_data) {
    ...
    array[0] = Share.parseShort(matched_data[0]);
    array[1] = Share.parseShort(matched_data[1]);
    array[2] = Share.parseShort(matched_data[2]);
    array[3] = Share.parseShort(matched_data[3]);
    return array;
  }
  ...
  public Object clone() {
    ...
    try {
      ...
      new_object.son_attribute =
          (this.son_attribute == null ? null : (SonSubtitleImageAttribute) son_attribute.clone());
  ...
  public SonSubtitleImageAttribute getCreateSonAttribute() {
    if (this.son_attribute == null) this.son_attribute = new SonSubtitleImageAttribute();
    return this.son_attribute;
  }

  public SonSubtitleImageAttribute getImageAttribute() {
    return this.son_attribute;
  }

  public void setImageAttribute(SubtitleImageAttribute attrib) {
    if (attrib instanceof SonSubtitleImageAttribute) {
      SonSubtitleImageAttribute son_attrib = (SonSubtitleImageAttribute) attrib;
      this.son_attribute = son_attrib;
    } // end if (attrib instanceof SonSubtitleImageAttribute)
  }
  ...
  public BufferedImage makeTransparentImage(BufferedImage img) {
    try {
      ...
      if (color_list == null) color_list = global_list.getColor();
      if (trans_list == null) trans_list = global_list.getContrast();

      SonSubtitleImageAttribute local_list = getImageAttribute();
      boolean is_using_local_color = !(local_list == null || local_list.colour == null);
      if (is_using_local_color) color_list = local_list.getColor(); // end if

      boolean is_using_local_trans = !(local_list == null || local_list.contrast == null);
      if (is_using_local_trans) trans_list = local_list.getContrast(); // end if

      Object[] color_tbl = header.color_table.toArray();

      BufferedImage tran_img = JImage.makeTransparentImage(img, color_list, trans_list, color_tbl);
  ...
  public String getHeaderAsString() {
    ...
    else return header.getHeaderAsString();
  }

  public void setHeader(Object header) {
    boolean ok = (header != null && (header instanceof SonHeader));
    if (ok) this.header = (SonHeader) header;
  } // public void setHeader(Object header)
    ...
    switch (col) {
      case 6:
        ...
        if (has_image) {
          ...
          return ico;
        } else return super.getData(row, col); // end if (has_image)
      default:
