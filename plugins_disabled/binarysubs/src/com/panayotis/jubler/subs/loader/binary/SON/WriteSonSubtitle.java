...
/**
 * This class writes SON index file, and images if the subtitle list is not already SON type. Index
 * file is always written to the chosen directory, but there is an option to write images to a
 * different set of directories, if created and chosen at the beginning of the routine. This option
 * is only available to non-SON subtitles. The number of images are divided equally over the number
 * of directories created/chosen. The user will need to create the directories using the
 * JFileChooser and select them from there. The set of directories chosen is NOT remembered in the
 * header, as this would violate the format's definition, but there is an option to find missing
 * images at the loading of the file.
 *
 * @see LoadSonImage
...
public class WriteSonSubtitle extends SubtitleUpdaterThread implements SONPatternDef {
  ...
  /**
   * Parameterised constructor. This constructor sstrip the incoming file of the '.tmp' extension,
   * as the filename already included the '.son' extension and there is no need for {@link
   * FileCommunicator#save} to rename the temporary file, since this code is executed in a separate
   ...
   * @param FPS Frame Per Second rate, often comes from the defaul preference (ie. 25)
   * @param encoding The text encoding scheme (ie. UTF-8).
   */
  public WriteSonSubtitle(
      JubFrame jb,
      Subtitles subtitle_list,
      JMaestroOptions moptions,
      File outfile,
      float FPS,
      String encoding,
      JPreferences prefs,
      WriteImageOptions writeOptionPane) {
    this.setJubler(jb);
    this.prefs = prefs;
   ...
   *
   * <ol>
   *   <li>Extract the header record from the first SON record in the subtitle list, and setup the
   *       header record with FPS, media-options, and the image output directory.
   *   <li>Write images (if the subtitle events have text without images).
   *   <li>Write the subtitle-index file.
   * </ol>
   *
  ...
  /**
   * Extract the header record from the first SON record in the subtitle list, and setup the header
   * record with FPS, media-options, and the image output directory.
   *
  ...
  /**
   * A string buffer is created to hold the entire textual content of the file. This buffer will be
   * written to the output file at the end of the routine. The header record is extracted from the
   * first element of the subtitle and it's string content is formed and stored in the string buffer
   * for final output. Every record in the subtitle list is accessed and asked to produce the string
   * version of its content. This textual presentation of the subtitle is appended to the output
   * buffer. The entire output buffer is flushed to the chosen ouput file.
   *
   * @param sub_list The list of subtitle events.
   * @param output_file The output file where the textual content of the subtitle list is written
   *     to.
   * @param encode The encoding scheme for the written file.
   * @return true if the process was carrie dout without errors, false otherwise.
   */
  ...
  /**
   * Writting images in a list of subtitle events out to their respective file, All images are
   * located at the same directory where the index file resides. If a subtitle event has text and do
   * not have an image, the text is converted to an image, and a file-name is automatically
   * generated. The image format, when converted from text, will be of Portable Network Graphic
   * (png) format. If the subtitle already contains an image, no image will be produced. There will
   * be the presence of a visual progress-bar showing the progress percentage, and the name of the
   * image file being written.
   *
   * @param sub_list The list of subtitle events.
  ...
  private void writeTextAsImage(JSubEditor ed, Subtitles sub_list, File output_file) {
    ...
    ImageFilenameGenerator gen = new ImageFilenameGenerator(sub_list, output_file, PNG_EXT);
    gen.generate(true);
    for (int i = 0; i < sub_list.size(); i++) {
      sonSubEntry = (SonSubEntry) sub_list.elementAt(i);
  ...
  private void writeImageAsPNGWhereImageDoesntExist(
      JSubEditor ed, Subtitles sub_list, File output_file) {
    /** Generate new files if needed, ie. when the filename is null */
    ImageFilenameGenerator gen = new ImageFilenameGenerator(sub_list, output_file, PNG_EXT);
    gen.generate();
    for (int i = 0; i < sub_list.size(); i++) {
  ...
  private void writeMixedTextAndImage(JSubEditor ed, Subtitles sub_list, File output_file) {
    ...
    for (int i = 0; i < sub_list.size(); i++) {
      ...
      if (has_text && has_image) writeImageIfNotThere();
      else if (has_text) makeSubPicture(sonSubEntry, sonSubEntry.getImageFile());
      else if (has_image) writeImageIfNotThere();
      try {
        String file_name = sonSubEntry.getImageFile().getName();
        ed.progressTooltip(file_name);
      } catch (Exception ex) {
  ...
  private void writeImageIfNotThere() {
    File f = sonSubEntry.getImageFile();
    if (!f.exists()) JImage.writeImage(sonSubEntry.getImage(), f, PNG_EXT);
  } // end private void writeImage()

  /**
   * Make a subtitle picture by drawing text onto an image, create an image file, update the
   * subtitle entry's content of the new image and file-name, then write the image to the crteated
   * file using 'PNG' format.
   *
