...
/**
 * This class process the TMPGenc format subtitle file. The structure contains following elements:
 *
 * <ul>
 ...
 * </blockquote>
 *
 * The subtitle file has header consists of '[LayoutData]' block and '[LayoutDataEx]' block, and the
 * subtitle events which are held in the block starting with the header line '[ItemData]'.
 *
 * <p>Entries in the [LayoutData] block hold definition for the subtitle-layout, determines how and
 * where the subtitle text will appears on screen when play-back, and each have a Name, DisplayArea,
 * FontName etc..
 *
 * <p>Entries in the [LayoutDataEx] block hold definition for global alignment setting, such as
 * centered or not, and reading direction (left-to-right or right-to-left). This function serves the
 * originator's market, that is the Japanese/English users.
 *
 * <p>Entries in the [ItemData] block consist elements such as subtitle ID, visibility (on/off),
 * start-time, end-time, layout index, and subtitle-text.
 *
 * <p>The subtitle text is held in a single line and thus encloses some elements that are needed to
 * parse when reading and replace when writing out to files. Mostly, the subtitle text are
 * surrounded with double-quotes, and for each double quote exists in the text, two instances are
 * stored. The line separator "\\n" is used to represent the single new-line '\n' character.
 *
 ...
 */
public class TMPGenc extends AbstractBinarySubFormat
  ...
  protected SubtitlePatternProcessor layoutDataItemProcessor,
      layoutDataExItemProcessor,
      subtitleEventProcessor;
  public static String extension = __("subtitle");
  public static String name = __("TMPGenc DVD Authoring");
  public static String extendedName = __("TMPGenc (subtitle)");
  ...
  public void recordCreated(SubtitleRecordCreatedEvent e) {
    ...
    if (isLayoutDataItemRecord) {
      ...
      if (layoutDataItemList == null) {
        ...
        if (header == null) header = new TMPGencHeaderRecord(); // if (header == null)
  ...
  public void preParsingDataLineAction(PreParsingDataLineActionEvent e) {
    ...
    processorList.remove(
        layoutDataItemProcessor, is_layout_data_ex_header_line || is_data_item_header_line);
  ...
  public Subtitles convert(Subtitles current_subs) {
    boolean is_convert = current_subs.isRequiredToConvert(TMPGencSubtitleRecord.class);
    if (is_convert) current_subs.convert(TMPGencSubtitleRecord.class, this.getClassLoader());
    return current_subs;
  } // public Subtitles convert(Subtitles current_subs)

  /**
   * This routine is used to write out subtitle records to a subtitle-file in the format of TMPGenc
   * format. A non-TMPGenc subtitle can inherit the default heading and records are written out in
   * the TMPGenc format, suitble for importing into the TMPGenc DVD Authoring solution.
   *
   * @param given_subs List of subtitle events
   * @param outfile The output-file. This file has 'temp' extension, but will be replaced after the
   *     routine is completed successfully. If not, the file with the chosen-name and 'temp'
   *     extension exists in the user's file system.
