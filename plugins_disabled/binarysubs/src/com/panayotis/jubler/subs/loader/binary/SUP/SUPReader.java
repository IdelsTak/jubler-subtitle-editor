...
public class SUPReader extends SUPCompressImageProcessor {
  ...
  private boolean getImageData(FileInputStream in) throws Exception {
    ...
    if (read_byte != RLEheader.length) return false;
    ...
    if (read_byte != packet_data.length) return false;
  ...
  private void decodingData() throws Exception {
    // color index 3,2 + 1,0
    int color_index_value = (0x00ff & sections[3]) << 8 | (0x00ff & sections[4]);
    // alpha index 3,2 + 1,0
    int alpha_index_value = (0x00ff & sections[6]) << 8 | (0x00ff & sections[7]);
    ...
    int duration = ((0x00ff & sections[22]) << 8) | (0x00ff & sections[23]);
    ...
    int min_x = (0x00ff & sections[9]) << 4 | (0x00f0 & sections[10] >>> 4);

    int max_x = (0x000f & sections[10]) << 8 | (0x00ff & sections[11]);

    int min_y = (0x00ff & sections[12]) << 4 | (0x00f0 & sections[13]) >>> 4;

    int max_y = (0x000f & sections[13]) << 8 | (0x00ff & sections[14]);
    ...
    // bottom_field x1, y1
    int bottom_field_pos = ((0x00ff & sections[18]) << 8) | (0x00ff & sections[19]);
    bottom_field_pos -= 4;
  ...
  public void processImageData() {
    ...
    try {
      in = new FileInputStream(processFile);
      while (ok) ok = getImageData(in); // end while
  ...
  public void readSupFile() {
    ...
    if (!ok) makeDefaultColourTable(); // end if (! ok)
    this.setRow(0);
    fireSubtitleUpdaterPreProcessingEvent();
    getImageData();
    fireSubtitleUpdaterPostProcessingEvent();
