...
/**
 * This class is responsible for checking the content and the status of each subtitle entry in a
 * subtitle list and gathers some basic statistics on them, checking to see if they have subtitle
 * texts only, have images only, have both, or some have and some don't. If entries have images, do
 * they have a file attached to it or not. Having a file means that the file entry must not null and
 * that the file exists and it is not a directory. This will help routines making decision how to
 * treat entries when operations are applied on them.
 *
...
public class ImageSubListInfo {
  ...
  public boolean hasTextOnlyButMix() {
    boolean yes = this.hasText() && this.hasNoText() && (!this.hasImage());
    return yes;
  }

  public boolean hasImageOnlyButMix() {
    boolean yes = this.hasImage() && this.hasNoImage() && this.hasNoText() && (!this.hasText());
    return yes;
  }
  ...
  public boolean checkInfo() {
    ...
    for (int i = 0; i < subList.size(); i++) {
      ...
      if (has_text) testFlag |= HAS_TEXT;
      else testFlag |= HAS_NO_TEXT;

      if (has_image) testFlag |= HAS_IMAGE;
      else testFlag |= HAS_NO_IMAGE;

      if (has_file) testFlag |= IMAGE_HAS_FILE;
      else testFlag |= IMAGE_HAS_NO_FILE;

      if (image_file_exists) testFlag |= IMAGE_FILE_EXISTS;
      else testFlag |= IMAGE_FILE_NOT_EXISTS;
