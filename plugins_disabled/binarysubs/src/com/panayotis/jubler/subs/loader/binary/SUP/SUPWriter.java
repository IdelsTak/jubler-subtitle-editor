...
public class SUPWriter extends SUPCompressImageProcessor {
   ...
   */
  public SUPWriter(JubFrame jubler, float fps, String encoding, File f) {
    super(jubler, fps, encoding);
    processFile = f;
  }

  /**
   * Checks to see if the entry is a image-type subtitle or not. If it is, checks to see if it
   * actually has an image. If not, generate a new-blank image, to avoid the rest of the subtitle
   * file being discarded by the process. The entry is then casted to the {@link ImageTypeSubtitle}
   * and return to the caller. <br>
   * <br>
   * When an entry is NOT a {@link ImageTypeSubtitle} entry, an instance of SON subtitle entry is
   * created and the subtitle text is converted to an image. When no text is found, a blank string
   * is used. The subtitle image will have a blue background, and the text image is over-laid in the
   * centre. The default header is held locally for next entries.
   *
   * @param entry The subtitle entry to be converted to an {@link ImageTypeSubtitle}.
  ...
  private ImageTypeSubtitle getImageTypeEntry(SubEntry entry) {
    ...
    if (!has_image) {
      ...
      } else img = JImage.getDefaultBlankImage();
  ...
  /**
   * This function will convert the image from the input instance of {@link ImageTypeSubtitle} to an
   * image with maximum of 4-colors. This is due to the SUP requirements for RLE compression that
   * the image will only have a maximum of 4 colors. Conversion is carried out here to avoid
   * color-cutoff that would happen in compression stage, which would produce wrong color indices,
   * resulting to a very bad image when it is decoded. The pixel data of image is extracted and held
   * locally so that it can be passed to the compressor later-on. The local user-color table is
   * updated with the new 4 colors found, either from the image or the color-palette list from the
   * color-reducer.
   *
   * @param img_entry The subtitle entry which contains image to be compressed.
   */
  ...
  /**
   * Creates an instance of {@link BitmapRLE} and pass it the local image data, its width and
   * height, and the color-table, before invokes its compress method to carry out the RLE
   * image-compression. The compressed data is retrieved and set locally.
   *
   * @return True if the compression was carried out without problems, false otherwise.
   */
  private boolean compressImageData() {
    ...
    } else compressedImageData = null; // end if (!ok)
    return ok;
  } // end private boolean compressImageData(Rectangle rec)

  /**
   * Sets the screen positions of the image on the screen in the control-section of the packet so
   * that it can be written to the file.
   *
    ...
    try {
      ...
      for (int a = 0; a < 4; a++)
        if (a < max_tbl_size) {
          ...
          pgc_alpha_value =
              0xF & Integer.parseInt(pgc_alpha_links[pgc_color_value].toString()) >>> 28;
  ...
  /** Runs through the subtitle list, compress the image and write it to a file. */
  public void writeSupFile() {
    ...
    try {
      ...
      for (int i = 0; i < sub_len; i++) {
        ...
        if (Share.isEmpty(img_entry)) break; // end if (Share.isEmpty(img_entry))

        getSubtitleImageData(img_entry);
        ok = compressImageData();
        if (!ok) break; // end if (!ok)
        ...
        try {
          ...
          SonSubtitleImageAttribute attrib =
              (SonSubtitleImageAttribute) img_entry.getImageAttribute();
          if (Share.isEmpty(attrib)) {
            attrib = new SonSubtitleImageAttribute();
            attrib.centreImage(img_entry.getImage());
          } else if (Share.isEmpty(attrib.display_area))
        ...
        if ((out.size() & 1) == 1) out.write((byte) 255);
        ...
        int in_frm_cnt = entry.getStartTime().getFrameCount(vfr);
