...
/**
 * Similar to SonSubEntry apart from the production of subtitle text during the output to files and
 * that entries are separated by a blank line. A typical example of SWT is shown below:
 *
 * <pre>
...
public class SWTSubEntry extends SonSubEntry {
  ...
  public String toString() {
    ...
    try {
      ...
      if (has_text) b.append(txt);

      String sub_text = this.getText();
      has_text = (sub_text != null && sub_text.length() > 0);
      if (has_text) b.append(sub_text).append(UNIX_NL); // end if
