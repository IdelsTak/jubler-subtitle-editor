...
public class BitmapRLE {
  ...
  /**
   * From the colour index input, checks to see if the item is already there. If not, check to see
   * if the length of pgc-index list exceed maximum 4 values or not.
   *
  ...
  private void setDecodedColor() {
    ...
    if (uncompressed_index < uncompressedData.length)
      uncompressedData[uncompressed_index] =
          last_used_color; // end if (uncompressed_index < uncompressedData.length)
    x += 1;
  ...
  private int setDecodedColor(int n, int c) {
    ...
    for (int i = 0; i < n; i++) setDecodedColor(); // end for (int i = 0; i < n; i++)

    if (y > this.height) return -1;
    else return 0; // end if (y > this.height)
  }
  ...
  private void decodeRLE(ArrayList<Byte> compressed_sequence) {
    ...
    if (is_end) return; // end while(! is_end)
    ...
    try {
      ...
      while (x < this.width) setDecodedColor(); // end for
    } catch (Exception ex) {
  ...
  /**
   * This routine section out the encoded sequence to a single-line (ie. the compressed pixels of a
   * single line of the picture, and the total uncompressed width should be the width of the
   * picture.), then calls {@link #decodeRLE} routine to uncompress the chosen sequence. The
   * end-of-line sequence should be "00 00" (hex bytes), but occasionally it preceded by an aligned
   * byte which is "00" as well. Encoded values should not have "00" bytes.
   *
  ...
  private void decodeRLE(int from, int to) {
    boolean is_end_line = false, is_end_file = false;
    int increment = 1;
    byte b1, b2, b3, b4;
    ...
    try {
      ...
      for (int i = from; i < to; i += increment) {
        ...
        if (is_end_line) {
          ...
          moveDecoderNextLine();
        } else compressed_sequence.add(b1);
  ...
  public boolean compress() {
    ...
    try {
      ...
      for (int i = 0, l = 0, a = 0, b = 0, color = 0; i < 2; i++) {
        // top_field first
        for (l = 0, color = 0, a = i * width; a < len; a += (2 * width)) {
          for (l = 0, color = 0, b = 0; b < width; b++, l++) {
