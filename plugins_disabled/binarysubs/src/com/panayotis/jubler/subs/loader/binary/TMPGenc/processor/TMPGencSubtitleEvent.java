 ...
 *   <li>Entry's visibility flag: 1=visible, 0=invisible. When 0 is set, the subtitle event is not
 *       visisible upon playback of DVD.
 ...
 *   <li>Subtitle-text: Surrounded by double-quotes, and two double-quotes refers to one instance of
 *       the double-quote in the text. New-line is replaced by the group "\\n" and thus the whole
 *       subtitle text resides in a single-line, so parsing and replacing must use '\\\\n'.
 * </ol>
 *
...
public class TMPGencSubtitleEvent extends SubtitlePatternProcessor implements TMPGencPatternDef {
  ...
  public void parsePattern(String[] matched_data, Object record) {
    try {
      ...
      if (txt.startsWith(char_double_quote)) txt = txt.substring(1);

      // remove the trailing double-quote (")
      int len = txt.length();
      if (txt.endsWith(char_double_quote)) txt = txt.substring(0, len - 1);
  ...
  /**
   * This is to cope with last line of the subtitle file where sub-text exists but without the
   * surrounding double-quotes, leading to error in the parsing of the pattern.
  ...
  private String getSubTextManually() {
    ...
    String split_pattern = char_double_quote + char_comma + digits + char_comma;
    String[] list = txt.split(split_pattern);
    boolean valid = (list.length == 2);
    if (valid) result_txt = list[1];
    else result_txt = ""; // end if (valid)
    return result_txt;
  }
}
