...
/**
 * This class is used to hold the header record of a SON subtitle format. The example for this block
 * of data in the subtitle file is shown here:
 *
 * <pre>
 * st_format	2
...
public class SonHeader implements SONPatternDef, Cloneable {
  ...
  /**
   * this is to flag that the record is a default header generated and further modification might be
   * required. This is useful when conversion from other record types are performed.
   */
  ...
  public void updateRowHeight(int height) {
    ...
    if (is_taller) max_row_height = height; // end if
  }
  ...
  public String toString() {
    ...
    if (tv_type.equals("PAL")) b.append(is_new ? "NON_DROP" : tape_type);
    else b.append(is_new ? "DROP" : tape_type);
    b.append(UNIX_NL);

    b.append("Pixel_Area").append("\t").append("(");
    if (is_new) b.append("0 477");
    else b.append(pixel_area[0]).append(" ").append(pixel_area[1]);
    b.append(")").append(UNIX_NL);

    b.append("Directory").append("\t");
    b.append(image_directory == null ? this.subtitle_file.getParent() : image_directory)
        .append(UNIX_NL);

    short w, h;
    if (is_new) {
      son_attribute = new SonSubtitleImageAttribute();
      try {
  ...
  public void addPaletteEntry(String index_s, String r_s, String g_s, String b_s) {
    short index = Share.parseShort(index_s);
    int r = Share.parseInt(r_s);
    int g = Share.parseInt(g_s);
    int b = Share.parseInt(b_s);
  ...
  public Object clone() {
    ...
    try {
      ...
      new_header.son_attribute =
          (son_attribute == null ? null : (SonSubtitleImageAttribute) son_attribute.clone());
      new_header.palletEntry =
          (palletEntry == null ? null : (Vector<SonPaletteEntry>) palletEntry.clone());
      ...
      new_header.color_table =
          (color_table == null ? null : (ArrayList<String>) color_table.clone());
    } catch (Exception ex) {
    }
    return new_header;
  }
  ...
  public void copyRecord(SonHeader o) {
    try {
      ...
      this.son_attribute =
          (o.son_attribute == null ? null : (SonSubtitleImageAttribute) o.son_attribute.clone());
      this.palletEntry =
          (o.palletEntry == null ? null : (Vector<SonPaletteEntry>) o.palletEntry.clone());
