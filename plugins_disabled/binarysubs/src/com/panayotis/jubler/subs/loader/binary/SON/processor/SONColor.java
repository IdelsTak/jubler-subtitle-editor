...
/**
 * This class is used to process the colour component of DVDMaestro format file. It process the
 * colour data entry in the following format:
 *
 * <pre>
 * Color	(0 1 6 7)
 * </pre>
 *
 * A short array is created for the values parsed, and is stored in either the {@link
 * SonSubEntry#son_attribute} or {@link SonHeader#son_attribute}, depending on the object currently
 * active.
 *
 * @see SonSubEntry
 * @see SonHeader
...
public class SONColor extends SubtitlePatternProcessor implements CommonDef {
  ...
  private static String pattern =
      "Color" + sp + "\\(" + sp_digits + sp + digits + sp + digits + sp + digits + sp_maybe + "\\)";
  ...
  /**
   * Default construction, set the pattern and the list of matching indices where data groups should
   * be found.
   */
  public SONColor() {
    super(pattern);
  ...
  /**
   * For the matched group of data, a short array is created and all digits representing selection
   * of colours
   *
   * @param matched_data The array of matched data, result of the parsing the pattern.
   * @param record The record currently active, it should be either an instance of either {@link
   *     SonHeader} or {@link SonSubEntry}.
   */
   ...
   *
   * @return Reference to the header record, null if the reference has not been set.
   */
