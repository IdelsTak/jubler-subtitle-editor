...
public class MPlayerSystemDependent extends SystemDependent {
  ...
  static void updateParameters() {
    ...
    if (version < 2 && (!params.equals(""))) {
      ...
      DEBUG.debug(
          "Updated configuration file. backup has been taken of the old configuration file .");
    }
  }
  ...
  public static String getDefaultMPlayerArgs() {
    ...
    if (IS_LINUX) font = " -fontconfig";
    else if (IS_WINDOWS) font = " -font " + System.getenv("SystemRoot") + "\\fonts\\arial.ttf";
    else {
      File freesans = new File(SystemFileFinder.AppPath, "lib/freesans.ttf");
      if (freesans.exists()) font = " -font %j/lib/freesans.ttf";
    }
  ...
  public static String[] getMPlayerEnvironment(MPlayer player) {
    if (!IS_MACOSX) return null;
    File resource =
        new File(player.getOptionsPanel().getExecFileName())
            .getParentFile()
            .getParentFile()
            .getParentFile()
            .getParentFile()
    ...
    if (!fontsconf.exists()) return null;
    ...
    for (Map.Entry<String, String> entry : variables.entrySet()) {
      ...
      if (name.equals("FONTCONFIG_PATH")) return null;
      env.add(name + "=" + value);
    }
  ...
  static String getMPlayerName() {
    if (IS_MACOSX) return "MPlayer OS Extended";
    return "MPlayer";
  }
}
