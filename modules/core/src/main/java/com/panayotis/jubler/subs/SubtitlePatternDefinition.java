...
/**
 * This class is used to hold the definition of a pattern that is used by the pattern matching
 * mechanism of Java. It's created based on the view that each subtitle data line is treated as
 * having a repeatable pattern, and that the parsing of the data using the pattern recognition is
 * much more effective. The workPattern is used for parsing, whilst the testPattern can be a
 * simplified version of the workPattern, although it's currently not being used.
 *
...
public class SubtitlePatternDefinition {
   ...
   * @param is_test_and_work_the_same Flag to set the work-pattern to be the same as test pattern.
   *     When this flag is true, the compiled work-pattern is used for the test-pattern as well.
   *     Both references are pointed to the same compiled Pattern object.
   */
  public SubtitlePatternDefinition(String work_pattern, boolean is_test_and_work_the_same) {
    setWorkPattern(work_pattern);
    if (is_test_and_work_the_same) testPattern = workPattern; // end if (is_test_and_work_the_same)
  }
   ...
   *
   * @param work_pattern The pattern to be compiled by the Pattern.compile()
   * @param test_pattern The pattern to be compiled by the Pattern.compile() and is used for testing
   *     purpose only
   */
