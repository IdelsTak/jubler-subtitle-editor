...
public interface CommonDef {
  ...
  public static final boolean IS_LINUX = OS.indexOf("linux") >= 0;
  public static final boolean IS_WINDOWS = OS.indexOf("windows") >= 0;
  public static final boolean IS_MACOSX = OS.indexOf("mac") >= 0;
  public static final String PROG_EXT = (IS_WINDOWS ? ".exe" : "");
  ...
  /**
   * Set up the path for tesseract's language files. This value could be null if the environment
   * variable 'TESSDATA_PREFIX' is not set, or it is set but the system is not refreshed properly.
   */
