...
public abstract class StyledTextSubFormat extends AbstractTextSubFormat {
  ...
  protected void parseSubText(SubEntry entry) {
    ...
    SubEv s;
    int startoffset =
        0; /* In order for not parsing the events when the text will be truncated, store the offset right here and substruct it on any new event */
    ...
    for (SubEv se : events) sbuf.delete(se.start, se.start + se.value.length() + 2);
            ...
            try {
              switch (sf.style.getType()) {
                ...
                case FORMAT_DIRECTION:
                  entry.setOverStyle(
                      sf.style,
                      ((HashMap<String, Direction>) sf.value).get(tag),
                      se.start,
                      se.start);
                  break;
                default:
  ...
  protected String rebuildSubText(SubEntry entry) {
    ...
    for (StyledFormat sf :
          getStylesDictionary()) { // Iterate for every supported format. Note: only supported
        // formats are saved
        over =
            entry.overstyle[sf.style.ordinal()]; // Get this AbstractStyleover for this format entry
        if (sf.storable && over != null)
          for (int j = 0;
              j < over.size();
              j++) { // Iterate through all events in this AbstractStyleover list
            ev = over.getVisibleEvent(j); // Get the value data
            switch (sf.style.getType()) {
              case FORMAT_FLAG:
                if (sf.value.equals(ev.value)) events.add(new SubEv(sf.tag, ev.position));
                break;
              case FORMAT_DIRECTION:
                String dirkey =
                    getDirectionKey((HashMap<String, Direction>) sf.value, (Direction) ev.value);
                if (dirkey != null) events.add(new SubEv(sf.tag + dirkey, ev.position));
                break;
              case FORMAT_COLOR:
                ...
                if (sf.style.equals(StyleType.FONTSIZE))
                  value =
                      String.valueOf(
                          Math.round(
                              ((Integer) ev.value)
                                  * getFontFactor())); // A hack to increase font size
  ...
  protected static String getDirectionKey(HashMap<String, Direction> dict, Direction dir) {
    ...
    for (Entry<String, Direction> en : dict.entrySet())
      if (en.getValue().equals(dir)) return en.getKey();
    return "";
  }
}
