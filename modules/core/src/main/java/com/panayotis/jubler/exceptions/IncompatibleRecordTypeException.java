...
/**
 * This class is used to raise exception condition when differences between two classes are found.
 * The difference should not based on the result of 'instanceof' test but on the absolute class-name
 * comparisons, as the 'instanceof' treats the implementation and inheritance as the same.
 *
...
public class IncompatibleRecordTypeException extends Exception {
  ...
  /**
   * The default message "Incompatible type detected". This message is used with the default
   * constructor.
   */
  ...
  /**
   * Constructs a new exception with the specified classes. The names of two classes are retrieved
   * and included in the message passing to the exception message, with an 'and' connector, to
   * indicate that the execption is raised due to the source of problem.
   *
