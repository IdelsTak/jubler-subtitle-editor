...
public class ShortcutsModel extends AbstractTableModel {
  ...
  private static final String colnames[] = {__("Command"), __("Key")};
  private static final int DEFAULT_MOD = SystemDependent.getDefaultKeyModifier();
  private static final int DISABLED_MOD = ~(KeyEvent.CTRL_MASK | KeyEvent.CTRL_DOWN_MASK);
  //
  ...
  public ShortcutsModel(JMenuBar bar) {
    ...
    for (int i = 0; i < bar.getMenuCount(); i++) {
      if (i > 0) original.add(null); // Add "---" between menus
    ...
    for (int i = 0; i < menu.getMenuComponentCount(); i++) {
      ...
      if (c instanceof JMenu) {
        ...
        if (item.getName() == null)
          DEBUG.debug(
              "Menu item \"" + prefix + item.getText() + "\" does not provide a valid name");
        else
          original.add(
              new MenuItem(prefix + item.getText(), item.getName(), item.getAccelerator()));
      }
    }
  }

  public void applyMenuShortcuts(JMenuBar bar) {
    for (int i = 0; i < bar.getMenuCount(); i++) applyItemsShortcuts(bar.getMenu(i));
  }
  ...
  private void applyItemsShortcuts(JMenu menu) {
    ...
    for (int i = 0; i < menu.getMenuComponentCount(); i++) {
      c = menu.getMenuComponent(i);
      if (c instanceof JMenu) applyItemsShortcuts((JMenu) c);
      else if (c instanceof JMenuItem) {
        JMenuItem item = (JMenuItem) c;
        if (item.getName() != null) {
          int index = current.indexOf(new MenuItem(null, item.getName(), (MenuItem.Shortcut) null));
          if (index >= 0) current.get(index).applyShortcut(item);
        }
      }
    }
  }
  ...
  public Object getValueAt(int row, int col) {
    ...
    if (col == 0) return entry == null ? "---" : entry.menuname;
    else return entry == null ? "" : entry.key.toString();
  }
  ...
  public void keyPressed(int keyid) {
    ...
    if (modifier > 0) buffer_mod |= modifier;
    else if (isValidKey(keyid)) {
      MenuItem old = current.get(current_id);
      current.set(
          current_id,
          new MenuItem(old.menuname, old.tag, new MenuItem.Shortcut(keyid, buffer_mod)));
  ...
  public void savePreferences() {
    ...
    for (int i = 0; i < current.size(); i++) {
      ...
      if (cur != null && (!cur.key.equals(orig.key))) keys.append(",").append(cur.toString());
    }
      ...
      try {
        MenuItem other =
            new MenuItem(
                null,
                m.group(1),
                new MenuItem.Shortcut(Integer.parseInt(m.group(2)), Integer.parseInt(m.group(3))));
  ...
  private String isValidCodes() {
    ...
    for (MenuItem s : original)
      if (s != null) {
        if (s.tag == null) return s.menuname;
        if (set.contains(s.tag)) return s.tag;
        if (s.tag.length() < 3) return "Tag too big: " + s.tag;
        set.add(s.tag);
      }
    return null;
  }
  ...
  private static class MenuItem {
    ...
    private MenuItem(String menuname, String tag, Shortcut key) {
      ...
      if (tag == null) throw new NullPointerException("Null tag for menu " + menuname);
      this.tag = tag;
    ...
    public boolean equals(Object obj) {
      if (obj instanceof MenuItem) return ((MenuItem) obj).tag.equals(tag);
      return false;
    }
    ...
    public void applyShortcut(JMenuItem item) {
      if (key.key == 0) item.setAccelerator(null);
      else item.setAccelerator(KeyStroke.getKeyStroke(key.key, key.modifier));
    }
