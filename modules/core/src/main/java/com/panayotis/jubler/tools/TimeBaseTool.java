...
public abstract class TimeBaseTool extends Tool {
  ...
  public boolean execute(JubFrame jub) {
    ...
    // Find affected list
    List<SubEntry> list;
    if (jub.isToolLocked()) list = Arrays.asList(selectedsubs);
    else list = getTimeArea().getAffectedSubs();
    if (list.isEmpty()) return false;
    getTimeArea().updateSubsMark(list);
    storeSelections();

    /* Perform tool */
    if (!affect(list)) return false;
  ...
  protected JTimeArea getTimeArea() {
    if (timepos == null)
      if (freeform) timepos = new JTimeFullSelection();
      else timepos = new JTimeRegion();
    return timepos;
  }
  ...
  protected JComponent getToolVisuals() {
    if (toolvisuals == null) toolvisuals = constructToolVisuals();
    return toolvisuals;
  }
