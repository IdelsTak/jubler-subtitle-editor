...
public class AutoSaver {
  ...
  public static final String AUTOSAVEPREFIX = "autosave.";
  private static final long AUTOSAVE_SECONDS = 30;
  private static final Random rnd;
  private static final File dir, olds;
  ...
  private static final TimerTask task =
      new TimerTask() {
        public void run() {
          ...
          /* cleanup old files */
          for (File current : olds.listFiles()) current.delete();
          olds.delete();
        }
      };
  ...
  public static File[] getAutoSaveListOnLoad() {
    ...
    if (olds.exists() && olds.isDirectory())
      for (File ofile : olds.listFiles()) ofile.renameTo(new File(dir, ofile.getName()));
  ...
  private static void deleteDirContents(File d) {
    ...
    try {
      for (File current : d.listFiles()) current.delete();
