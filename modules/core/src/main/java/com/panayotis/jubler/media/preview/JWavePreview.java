...
public class JWavePreview extends JPanel implements DecoderListener {
  ...
  private static final Color[] background = {new Color(0, 20, 0), new Color(0, 20, 20)};
  private static final Color bordercolor = Color.WHITE;
  private static final Color basecolor = Color.LIGHT_GRAY;
  ...
  private void updateWave() {
    if (isEnabled() && mfile != null) audio = mfile.getAudioPreview(start_time, end_time);
    else audio = null;
    if (audio == null) audio = demoaudio;
    if (is_maximized) audio.normalize();

    /* Remove old panels */
    if (panels != null)
      for (int i = 0; i < panels.length; i++) if (panels[i] != null) remove(panels[i]);
    /* Create new panels */
