...
public abstract class AbstractGenericTextSubFormat extends SubFormat {
  ...
  public Subtitles parse(String input, float FPS, File f) {
    try {
      if (!isSubtitleCompatible(input)) return null; // Not valid - test pattern does not match
      DEBUG.debug("Found file " + getExtendedName());
      subtitle_list = new Subtitles();
      input = initLoader(input);
      for (SubEntry entry : loadSubtitles(input)) subtitle_list.add(entry);
  ...
  public boolean produce(Subtitles subs, File outfile, MediaFile media) throws IOException {
    ...
    if (is_convert) subs.convert(SubEntry.class, getClassLoader()); // end if (is_convert)
    initSaver(subs, media, res);
    for (int i = 0; i < subs.size(); i++) appendSubEntry(subs.elementAt(i), res);
    cleanupSaver(res);
    ...
    // encoder =
    // Charset.forName(jub.prefs.getSaveEncoding()).newEncoder().onMalformedInput(CodingErrorAction.REPORT).onUnmappableCharacter(CodingErrorAction.REPORT);
    CharsetEncoder encoder = Charset.forName(ENCODING).newEncoder();

    BufferedWriter out =
        new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outfile), encoder));
  ...
  protected void updateAttributes(
      String input, Pattern title, Pattern author, Pattern source, Pattern comments) {
    Matcher m;
    ...
    for (int i = 0; i < attrs.length; i++)
      if (attrs[i] != null && attrs[i].equals("")) attrs[i] = null;
