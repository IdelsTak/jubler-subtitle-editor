...
public class SubStyle implements Comparable {
  ...
  public static final Integer[] FontSizes = {
  ...
  public SubStyle(String name) {
    ...
    for (int i = 0; i < types.length; i++) values[i] = types[i].getDefault();
  }
  ...
  public void set(StyleType which, Object what) {
    ...
    if (what instanceof String) values[where] = which.init((String) what);
    else values[where] = which.init(what);
  }
  ...
  class UniqName {
    ...
    private boolean findNameInList(NameList list, Object obj) {
      for (int i = 0; i < list.size(); i++)
        if (list.getElementAt(i) != obj && list.getNameAt(i).equals(newname)) return true;
      return false;
    }
    ...
    private void normalizeInternalName(NameList list, Object obj) {
      ...
      for (int i = 0; i < list.size(); i++) {
        ...
        if (list.getElementAt(i) != obj
            && other.numb_name >= numb_name
            && other.text_name.equals(text_name))
          /* update number */
