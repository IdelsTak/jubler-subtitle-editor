...
public class Share implements CommonDef {
   ...
   * @return The file-name without the extension if it has any, null if there are errors.
   */
   ...
   *
   * @param f {@link File} to be extracted.
   * @return A string represent the file's extension with a leading dot character(.), if there is
   *     one. Null otherwise.
   */
  ...
  public static File browseDir(String file_name, File start_directory) {
    ...
    try {
      ...
      if (option == JFileChooser.APPROVE_OPTION) {
        ...
        if (accepted_dir.isFile()) accepted_dir = accepted_dir.getParentFile(); // end if
      } // end if
    } catch (Exception ex) {
    }
    return accepted_dir;
   ...
   * @return The number of words contains in the text line, 0 if no words are found or an error
   *     occurred.
   */
  ...
  public static boolean changeDir(Subtitles subs, int from, File new_dir) {
    ...
    try {
      ...
      for (int i = from; i < size; i++) {
        ...
        if (entry == null) throw new IncompatibleRecordTypeException();
  ...
  public static ImageTypeSubtitle getImageSubtitleEntry(SubEntry entry) {
    if (entry instanceof ImageTypeSubtitle) return (ImageTypeSubtitle) entry;
    else return null;
