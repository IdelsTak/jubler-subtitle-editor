...
public abstract class AbstractStyleover extends ArrayList<AbstractStyleover.Entry> {
  ...
  public AbstractStyleover setMaxStylePosition(int pos) {
    ...
    for (int i = size() - 1; i >= 0; i--) {
      entry = get(i);
      if (entry.prev != null)
        if (entry.prev.position > pos) remove(i);
        else if (entry.next != null && entry.next.position > pos) entry.next = null;
    }
  ...
  public void updateClone(AbstractStyleover old) {
    ...
    for (AbstractStyleover.Entry entry : old) add(new AbstractStyleover.Entry(entry));
  }
  ...
  public void setStyleType(Object type) {
    styletype = type;
  }

  /**
   * Find the event index which is just to the left of the current postion Note that the "NEXT"
   * object is more importand than the PREV object
   */
  private StyleoverEvent findPrevEvent(int position, Object basic) {
    ...
    for (AbstractStyleover.Entry entry : this) {
      ...
      if (entry.prev.position > position) break;
  ...
  public Object getValue(int start, int end, Object basic, String subtext) {
    ...
    for (AbstractStyleover.Entry entry : this) {
      ...
      if (entry.prev.position > start && entry.prev.position < end) return null;
      if (entry.prev.position >= end && entry.prev.position != 0) break;
  ...
  /**
   * Search to the left for the next event. If it has the same value then set start to the found
   * item event start and return. If it does not have the same value then search to the left for the
   * real beggining of the event (same, if character based, beginning of paragraph if paragraph
   * based) if the style should change up to the beginning of the paragraph, it will dump
   * intermediate events
   */
  private StyleoverEvent makeStartEvent(Object newvalue, int start, Object basic, String txt) {
    ...
    return new StyleoverEvent(
        newvalue, (prev_edge > prev_style.position) ? prev_edge : prev_style.position);
  }

  /**
   * Create a new style based of the current one at this position. If the previous style is actually
   * the startevent, then there is no end event (return null)
   *
   * <p>Or else again search to the right for the real end (same rules as findStart apply). if the
   * style should change up to the end of the paragraph, it will dump intermediate events
   */
  private StyleoverEvent makeEndEvent(
      Object newvalue, int start, int end, Object basic, String txt) {
  ...
  public void addEvent(Object event, int start, int end, Object basic, String txt) {
    ...
    if (endevent != null) { // We do not want hanging invisible styles (again)
      ...
      if (endentry.prev != null) endentry.next = endevent;
      else {
  ...
  /**
   * Clear all events in (start, end). Leave border items at it's place - they will be needed later
   * on
   */
  ...
  public void insertText(int start, int length) {
    ...
    for (int i = 0; i < size(); i++) { // Parse all elements
      ...
      if (entry.prev.value.equals(
            entry
                .next
                .value)) // And it has the same values prev & next (which means that we wan to copy
          // the rigth style to the left
          entry.next = null;
        else { // We need to create a new event
          AbstractStyleover.Entry splitnext =
              new AbstractStyleover.Entry(
                  new StyleoverEvent(entry.next.value, start + length), null);
          entry.next = null;
          i++;
          add(i, splitnext);
        }
      else if (entry.prev.position >= offset_start
          && entry.prev.position
              != 0) { // All other events are further up, so we need to shift their time (except for
        // the zero event)
  ...
  public void cleanupEvents(Object basic, String subtext) {
    ...
    for (int i = 0; i < size(); i++) {
      ...
      if (entry.prev.value.equals(data)
          || deleteDependingOnStyle(entry, subtext)
          || entry.prev.position == subtext.length()) remove(i);
      else {
        /* OK, this entry is correct */
        olddata =
            data; //  This entry is used for storing the real data value, if we have more entried in
        // the same position than 1
  ...
  public void removeText(int start, int length, int textlength, Object basic, String subtext) {
    ...
    for (int i = size() - 1; i >= 0; i--) {
      ...
      if (entry.prev.position >= start) {
        if (entry.prev.position <= (start + length)) entry.prev.position = start;
        else entry.prev.position -= length;
  ...
  /* NOTE: since we can not handle unknown events, this method does NOTHING to this kind of events */
  public static void applyAttributesToDocument(
      StyledDocument doc, Object defaultval, AbstractStyleover over, int textsize) {
  ...
  private static void applyAtributesToSection(
      StyledDocument doc, Object type, Object value, int from, int length) {
    SimpleAttributeSet set = new SimpleAttributeSet();

    if (type == StyleConstants.Alignment) {
      if (value == Direction.TOPRIGHT || value == Direction.RIGHT || value == Direction.BOTTOMRIGHT)
        value = StyleConstants.ALIGN_RIGHT;
      else if (value == Direction.TOPLEFT
          || value == Direction.LEFT
          || value == Direction.BOTTOMLEFT) value = StyleConstants.ALIGN_LEFT;
