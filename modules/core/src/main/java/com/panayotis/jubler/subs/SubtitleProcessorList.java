...
/**
 * This class is used to hold the list of {@link SubtitlePatternProcessor}s, each will be in charge
 * of a recognising and parsing a data line. There are a group of action events to be fired:
 *
 * <ul>
 *   <li>{@link PreParsingDataLineActionEvent} Occurs before the data line is parsed. It is possible
 *       to set flag to ignore the data line using {@link #setIgnoreData setIgnoreData}
 *   <li>{@link SubtitleRecordCreatedEvent} Occurs before the data line is parsed and when either
 *       {@link #isCreateNewObject isCreateNewObject} flag is set or the reference to the target
 *       object of the {@link SubtitlePatternProcessor} is "null". Initialisation to the created
 *       object or changing the state of the {@link SubtitlePatternProcessor} are possible using
 *       this event. Record creation is possible using the name of the target class, therefore the
 *       {@link SubtitlePatternProcessor#getTargetObjectClassName getTargetObjectClassName} is used.
 *       As this uses the <code>Class.forname(String).newInstance()</code>, the default constructor
 *       of the target class is called and no parameterised constructors are recognised.
 *   <li>{@link ParsedDataLineEvent} Occurs after the data line has been parsed by a {@link
 *       SubtitlePatternProcessor}. Resetting of the {@link #isCreateNewObject isCreateNewObject}
 *       flag is possible within this event to commence the creation of a new record on the next
 *       parsing turn.
 * </ul>
 *
 * The {@link #parse parse} routine also removes the {@link SubtitlePatternProcessor} from the list
 * after it has completed parsing the data line and it's set to be {@link
 * SubtitlePatternProcessor#isRemovable isRemovable}. This is to avoid the unnecessary testing of
 * pattern in the next run, when the pattern is known to be occurred only once.
 *
 * <p>Voluntary removal of processors is possible within processing events, for instance after a
 * certain point of processing occured. Instance of {@link SubtitlePatternProcessor} must have
 * access to the {@link SubtitleProcessorList} in order to do this.
 *
 * <p>The {@link #parse parse} routine terminates after an instance of {@link
 * SubtitlePatternProcessor} processed its data.
 *
...
public class SubtitleProcessorList extends ArrayList<SubtitlePatternProcessor> {
  ...
  /**
   * A copy of processors and their orders are kept in this list. This list is used to restore the
   * list of processors when the parsing of a data file is completed, ready for the next data file.
   * It is doing so to avoid the removed processors will be missed in the next task.
   */
  private ArrayList<SubtitlePatternProcessor> patternProcessorListCopy =
      new ArrayList<SubtitlePatternProcessor>();

  private ArrayList<SubtitleRecordCreatedEventListener> recordCreatedEventList =
      new ArrayList<SubtitleRecordCreatedEventListener>();
  private ArrayList<ParsedDataLineEventListener> parsedEventList =
      new ArrayList<ParsedDataLineEventListener>();
  private ArrayList<PreParsingDataLineActionEventListener> preParsingEventList =
      new ArrayList<PreParsingDataLineActionEventListener>();
  ...
  private ArrayList<SubtitlePatternProcessor> removeList =
      new ArrayList<SubtitlePatternProcessor>();
  ...
  public SubtitleProcessorList(
      SubtitlePatternProcessor processor, String input_data, float FPS, File input_file) {
    this.add(processor);
  ...
  public void fireSubtitleRecordCreatedEvent() {
    ...
    for (int i = len - 1; i >= 0; i--) {
      SubtitleRecordCreatedEvent event =
          new SubtitleRecordCreatedEvent(this, ActionEvent.ACTION_PERFORMED, "Record Created");

      SubtitleRecordCreatedEventListener e = this.recordCreatedEventList.get(i);
  ...
  public void fireSubtitleDataParsedEvent() {
    ...
    for (int i = len - 1; i >= 0; i--) {
      ParsedDataLineEvent event =
          new ParsedDataLineEvent(this, ActionEvent.ACTION_PERFORMED, "Parsed Data Line");

      ParsedDataLineEventListener e = this.parsedEventList.get(i);
  ...
  public void fireSubtitleDataPreParsingEvent() {
    ...
    for (int i = len - 1; i >= 0; i--) {
      PreParsingDataLineActionEvent event =
          new PreParsingDataLineActionEvent(
              this, ActionEvent.ACTION_PERFORMED, "Pre-Parsing Data Line");

      PreParsingDataLineActionEventListener e = this.preParsingEventList.get(i);
      event.setProcessor(getCurrentProcessor());
  ...
  public void setAllTargetObjectClassNameAndRemovable(
      Collection<SubtitlePatternProcessor> processorCollection, String name, boolean removable) {
    this.setAllTargetObjectClassName(processorCollection, name);
    this.setAllTargetObjectRemovable(processorCollection, removable);
  }

  public void setAllTargetObjectClassName(
      Collection<SubtitlePatternProcessor> processorCollection, String className) {
  ...
  public void setAllTargetObjectRemovable(
      Collection<SubtitlePatternProcessor> processorCollection, boolean removable) {
    boolean valid = !(processorCollection == null || processorCollection.size() == 0);
    if (valid) {
      Iterator<SubtitlePatternProcessor> it = processorCollection.iterator();
      while (it.hasNext()) {
  ...
  public void setAllTargetObjectClassNameAndRemovable(
      SubtitlePatternProcessor[] processorCollection, String name, boolean removable) {
    setAllTargetObjectRemovable(processorCollection, removable);
    setAllTargetObjectClassName(processorCollection, name);
  }

  public void setAllTargetObjectRemovable(
      SubtitlePatternProcessor[] processorCollection, boolean removable) {
  ...
  public void setAllTargetObjectClassName(
      SubtitlePatternProcessor[] processorCollection, String name) {
    boolean valid = (processorCollection != null && processorCollection.length > 0);
    for (int i = 0; valid && i < processorCollection.length; i++) {
      SubtitlePatternProcessor ps = processorCollection[i];
  ...
  public void setAllTargetObjectClassName(String name) {
    Iterator<SubtitlePatternProcessor> it = this.iterator();
    while (it.hasNext()) {
      SubtitlePatternProcessor ps = it.next();
      ps.setTargetObjectClassName(name);
  ...
  public void setAllTargetObject(
      SubtitlePatternProcessor[] processorCollection, Object target_object) {
    boolean valid = (processorCollection != null && processorCollection.length > 0);
    for (int i = 0; valid && i < processorCollection.length; i++) {
      SubtitlePatternProcessor ps = processorCollection[i];
  ...
  public void setAllTargetObject(
      Collection<SubtitlePatternProcessor> processorCollection, Object target_object) {
    Iterator<SubtitlePatternProcessor> it = processorCollection.iterator();
    while (it.hasNext()) {
      SubtitlePatternProcessor ps = it.next();
      ps.setTargetObject(target_object);
  ...
  public void parse() {
    try {
      for (int i = 0; i < this.size(); i++) {
        SubtitlePatternProcessor ps = this.get(i);
        ...
        if (is_create_new) {
          ...
          if (is_empty) {
            String msg =
                __("Cannot create new record. Processor: [")
                    + ps.getClass().getName()
                    + __("]. Reason: target object class name is empty.");
        ...
        if (is_remove) removeList.add(ps); // end if (is_remove)
        break;
  ...
  /**
   * Add a instance of {@link SubtitlePatternProcessor} to the removable list, but only doing so if
   * the instance exists in the processor list.
   *
  ...
  public void remove(SubtitlePatternProcessor ps) {
    ...
    if (is_remove) this.removeList.add(ps); // if (is_remove)
  } // end public void remove(SubtitlePatternProcessor ps) {

  /**
   * Add a instance of {@link SubtitlePatternProcessor} to the removable list under the condition
   * provided, but only doing so if the instance exists in the processor list.
   *
   * @param ps The processor to add to removable list.
   * @param condition true if the processesor is to be added to the removable list, false otherwise.
   */
  ...
  public ArrayList<SubtitlePatternProcessor> getRemoveList() {
    return removeList;
  }
