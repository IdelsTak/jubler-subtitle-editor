...
public abstract class TerminalViewport extends ExternalViewport {
  ...
  protected void sendData(String data) throws IOException {
    if (cmdpipe == null) throw new IOException("Can not receive commands, pipe non existent.");
    cmdpipe.write(data + "\n");
    cmdpipe.flush();
  }
  ...
  public void start() throws ExtProgramException {
    try {
      player.cleanUp(); // Make sure player is in it's initial position (i.e. no subtitle files
      // hanging around)

      if (!player.isValid())
        throw new ExtProgramException(
            "Unable to initialize player '"
                + player.getDescriptiveName()
                + "'\nPlease make sure '"
                + player.getDescriptiveName()
                + "' path is properly defined under 'Preferences'.");
    ...
    public void run() {
      ...
      try {
        while ((info = outpipe.readLine()) != null) {
          ...
          if (info != null) DEBUG.debug(player.getName() + "> " + info);
        }
      } catch (Exception e) {
      }
