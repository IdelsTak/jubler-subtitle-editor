...
public class AudioPreview {
  ...
  public AudioPreview(float[] data) {
    ...
    if ((data.length % (length * 2)) != 0)
      throw new ArrayIndexOutOfBoundsException(
          __("Trying to intialize audio preview with wrong size {0}", data.length));
  ...
  public static boolean isAudioPreview(File cfile) {
    ...
    try {
      ...
      for (int i = 0; i < 7; i++) header.append((char) file.readByte());
      file.close();
    } catch (IOException e) {
      DEBUG.debug(e);
    }
  ...
  public void normalize() {
    ...
    for (int channel = 0; channel < cache.length; channel++) {
      ...
      for (int sample = 0; sample < cache[channel].length; sample++) {
        if (max < cache[channel][sample][0]) max = cache[channel][sample][0];
        if (min > cache[channel][sample][1]) min = cache[channel][sample][1];
      }
      min = 0.5f - min;
      max -= 0.5f;
      float factor = 0.5f / Math.max(min, max);
      if (factor > 254.5f) factor = 0;
