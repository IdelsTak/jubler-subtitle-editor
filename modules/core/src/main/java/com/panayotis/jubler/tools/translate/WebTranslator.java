...
public abstract class WebTranslator implements Translator, ActionListener {
  ...
  public boolean translate(
      final List<SubEntry> subs, final String from_language, final String to_language) {
    errorstream = "";
        ...
        new Thread() {
          ...
          public void run() {
            for (int i = 0; i < subs.size(); i += blocksize) {
              ...
              errorstream =
                  translatePart(
                      subs, i, Math.min(subs.size(), i + blocksize), from_language, to_language);
              if (errorstream != null) {
                JIDialog.error(
                    null,
                    __("Translating failed with error:") + '\n' + errorstream,
                    __("Error while translating subtitles"));
  ...
  protected abstract String getTranslationURL(String from_language, String to_language)
      throws MalformedURLException;

  protected abstract boolean isProtocolPOST();

  protected abstract String getConvertedSubtitleText(List<SubEntry> subs)
      throws UnsupportedEncodingException;

  protected abstract String parseResults(List<SubEntry> subs, BufferedReader in) throws IOException;

  private String translatePart(
      List<SubEntry> subs, int fromsub, int tosub, String from_language, String to_language) {
    ...
    try {
      ...
      URL req =
          new URL(
              getTranslationURL(from_language, to_language) + (isProtocolPOST() ? "" : "&" + txt));
  ...
  private List<SubEntry> getSubtitlesGroup(List<SubEntry> subs, int from, int to) {
    ...
    for (int i = from; i < to; i++) part.add(subs.get(i));
    return part;
  }
}
