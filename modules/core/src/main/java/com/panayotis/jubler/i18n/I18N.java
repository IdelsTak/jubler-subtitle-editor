  ...
  private static void populateLanguage() {
    ...
    if (ls.equals("en")) {
      ...
      for (String p :
        ...
        if (json.isFile()) {
          try {
            Json.parse(new FileReader(json))
                .asObject()
                .forEach(member -> transl.put(member.getName(), member.getValue().asString()));
            DEBUG.debug(
                "Using language " + json.getName().substring(0, json.getName().length() - 5));
            return;
