...
public class SubFile {
  ...
  private static final String[] basic_encodings = {"UTF-8", "ISO-8859-1", "UTF-16"};
  private static final String basic_fileencoding = basic_encodings[0];
  private static final float basic_FPS = 25f;
  private static final SubFormat basic_format;
  ...
  static {
    ...
    if (f == null) f = Availabilities.formats.findFromName("SubRip");
    if (f == null) f = Availabilities.formats.findFromName("PlainText");
    basic_format = f;
  }
  ...
  public static void setDefaultEncoding(int i, String enc) {
    if (enc == null) enc = basic_encodings[i];
    def_encodings[i] = enc;
  }
  ...
  public SubFile(String encoding, float FPS, SubFormat format, File file, boolean extension) {
    ...
    setFPS(FPS);
    setFormat(format);
    if (extension == EXTENSION_GIVEN) setFile(file);
    else setStrippedFile(file);
  }
  ...
  public SubFile(String pack) throws InstantiationException {
    this();
    if (pack == null || pack.equals("")) throw new InstantiationException();
    ...
    if (fps_pos < 0) throw new InstantiationException();
    int file_pos = pack.indexOf(";", fps_pos + 1);
    if (file_pos < 0) throw new InstantiationException();
  ...
  public void setEncoding(String enc) {
    if (enc == null || enc.equals("")) enc = basic_fileencoding;
    encoding = enc;
  }
  ...
  public void setFormat(SubFormat format) {
    if (format == null) format = basic_format;
    this.format = format;
  }
  ...
  public void setFormat(String format) {
    ...
    if (this.format == null) this.format = Availabilities.formats.findFromName(format);
    if (this.format == null) this.format = basic_format;
  }
  ...
  public void setStrippedFile(File f) {
    if (f == null) f = new File(FileCommunicator.getDefaultDirPath() + __("Untitled"));
    savefile_noext = f;
  ...
  public boolean equals(Object o) {
    if (o != null && (o instanceof SubFile)) return savefile.equals(((SubFile) o).savefile);
    return false;
  }
}
