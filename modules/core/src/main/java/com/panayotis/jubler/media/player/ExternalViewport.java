...
public abstract class ExternalViewport implements Viewport {
  ...
  private static final int SEEK_OFFSET = -3;
  ...
  protected abstract void sendData(String data)
      throws IOException; // Low level command to send data to the binary
  ...
  protected boolean sendCommands(String[] com) {
    ...
    for (int i = 0; i < com.length; i++) {
      ...
      try {
        if (com[i] != null) sendData(com[i] + "\n");
      } catch (IOException e) {
        if (!quit_is_pending) quit();
        return false;
      }
    }
    return true;
