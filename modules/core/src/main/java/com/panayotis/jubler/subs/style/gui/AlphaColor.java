...
public class AlphaColor extends Color {
  ...
  public AlphaColor(String color) {
    super(
        Integer.parseInt(
            color.substring((color.length() > 6) ? color.length() - 6 : 0, color.length()), 16));
  ...
  public Color getMixed(Color other, int newalpha) {
    ...
    float na = 1 - a;
    return new Color(
        calcColor(getRed(), other.getRed(), a, na),
  ...
  public String toString() {
    ...
    return "00".substring(0, 2 - alp.length())
        + alp
        + "000000".substring(0, 6 - res.length())
        + res;
  }
