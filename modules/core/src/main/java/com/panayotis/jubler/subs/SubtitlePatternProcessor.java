...
/**
 * This is a template for pattern processor. It's incharge of pattern matching, using the
 * pre-defined pattern (the workPattern), and returns the matched groups of data. The orginal list
 * of group - from the Pattern.matcher() - is returned if the {@link #matchIndexList} is null,
 * otherwise indexes from the list are used to obtain the parsed data, resulting in a zero-based
 * ordered list of the matching results, so instead of having to access to individual data item
 * using the index from the original group list, programmers can use zero as the first item, one for
 ...
 * </blockquote>
 *
 * To work out the indexes, either count the patterns, or do not set index at all, leaving it to
 * null. The pattern matched result will return in the array <code>matched_data</code>.
 *
...
public abstract class SubtitlePatternProcessor {
   ...
   *
   * @param pattern The regular expression string to be compiled by the Pattern.compile() method.
   * @param index The index at which the result list of matched groups is to be accessed when the
   *     data is found.
   * @param targetObjectName The fully qualified name of the class object that the parsing routine
   *     will use to create in order to pass to the implementation of parse() method. The creation
   *     of the object is controlled by a flag.
   ...
   *
   * @param pattern The regular expression string to be compiled by the Pattern.compile() method.
   * @param index The index at which the result list of matched groups is accessed when the data is
   *     found.
   * @param targetObjectName The fully qualified name of the class object that the parsing routine
   *     will use to create in order to pass to the implementation of parse() method. The creation
   *     of the object is controlled by a flag.
   * @param removable Flag to indicate if the processor is to be removed from the list of processors
   *     once the data is recognised and parsed. This is used for data patterns that are ocurred
   *     once only within the subtitle file.
   */
  public SubtitlePatternProcessor(
      String pattern, int index, String targetObjectName, boolean removable) {
   ...
   *
   * @param pattern The regular expression string to be compiled by the Pattern.compile() method.
   * @param index_list The list of indexes at which the result list of matched groups is accessed
   ...
   */
  public SubtitlePatternProcessor(
      String pattern, int[] index_list, Object target_object, boolean removable) {
    this(pattern, index_list);
  ...
  /**
   * Creates the pattern definition from the pattern string and set the working, testing pattern to
   * be the same as
   *
   * @param pattern
   */
  ...
  public String[] matchPattern() {
    try {
      ...
      if (has_data) {
        ...
        if (has_index_list) return this.matchPattern(getTextLine(), getMatchIndexList());
        else return this.matchPattern(getTextLine()); // end if if (has_index_list)
      } // end if (has_data)
    } catch (Exception ex) {
  ...
  /**
   * This routine matching the working pattern that was defined against the input text line and
   * return the array of matched groups of data within the input. The group of matched data is
   * obtained by the
   *
   * @param input
  ...
  public String[] matchPattern(String input) {
    ...
    try {
      ...
      if (is_found) {
        ...
        for (int k = 0; k < group_count; k++) getFoundList()[k] = m.group(k); // end for
  ...
  /** @return The array of matching indices, null if the reference has not been set. */
  ...
  /** @return The textual input data of the file, null if the reference has not been set. */
  ...
  /** @return The reference to the target object, null if the object has not been created. */
