...
public class AvailSubFormats {
  ...
  public boolean hasMoreElements() {
    if (current < Formats.size()) return true;
    return false;
  }
  ...
  public SubFormat findFromName(String ext) {
    ...
    for (int i = 0; i < Formats.size(); i++)
      if (Formats.get(i).getName().equals(ext)) return Formats.get(i);
    return null;
  }

  /**
   * Find a single instance of handler that handle the given extension. Since the same file
   * extension can be used by multiple formats, the routine will return null when more than one
   * instances are found.
   *
   * @param ext The subtitle file's extension given.
   * @return unique file handler for the given extension, null if not found or there are more than
   *     one handler which can handle the same file extension.
   */
  public SubFormat findFromExtension(String ext) {
    ...
    for (SubFormat found_format : Formats) {
      ...
      if (is_found) found_list.add(found_format); // end if
    } // end for (SubFormat found_format : Formats)
    is_found = (found_list.size() == 1);
    if (is_found) format = found_list.get(0); // end if (is_found)
    return format;
  ...
  /**
   * Add one instance of the format into the list and sort the instances into the order number.
   *
