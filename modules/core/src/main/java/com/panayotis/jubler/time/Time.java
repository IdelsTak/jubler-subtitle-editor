...
public final class Time implements Comparable<Time>, Cloneable, CommonDef {
  ...
  /**
   * This is to generate end-time, using starting frame-count and the duration in the 10th.
   *
  ...
  /**
   * This is to generate the start-time from a frame-count, that is says 25 frames/second, 1500
   * frames/minute and 90,000 frames/hour. By dividing the frames to 90, the value is suitable for
   * Date, which uses milliseconds, however the millisecond part needs to convert by (mill * 90 /
   * frame_rate) to get the time-equipvalent.
   *
  ...
  private void setTime(String h, String m, String s, String f) {
    ...
    try {
      ...
      sec = Short.parseShort(s);
      flength = f.length();
      if (flength < 3) f = f + "000".substring(flength);
      milli = Short.parseShort(f);
      setTime(hour, min, sec, milli);
    } catch (NumberFormatException e) {
      invalidate();
  ...
  private void setMilliSeconds(int msecs) {
    if (msecs < 0) msecs = 0;
    if (msecs > MAX_MILLI_TIME) msecs = MAX_MILLI_TIME;
    this.msecs = msecs;
  }
  ...
  /**
   * Converts the time that includes the frame-count in the time-value by extracts the millisecond
   * part and convert it appropriately to time unit. The time input must have been divided by 90,
   * (ie. taken from 90,000 frames per hour), before passing to this routine.
   *
   ...
   * @return The string representing time format, suitable to be parsed by a DateFormat instance.
   */
  private String formatTime(long time_value, float frame_rate, boolean is_to_time) {
    ...
    int milli_part = 0;
    if (is_to_time) milli_part = (int) (n1 * 90f / frame_rate);
    else milli_part = (int) (n1 * frame_rate / 90f);

    return (time_sub_str + milli_part);
  }

  /**
   * This routine return the number of frames/second, given that the the internal milliseconds used
   * to store this value. This takes the default 90,000 frames per hour, and since it was stored in
   * the milliseconds, the value stored will be multiplied by 90.
   *
  ...
  /**
   * Inserting zeros into the back of the value input, based on the total length required. If the
   * length of the original is equal or longer than the total length required, the original is
   * returned.
   *
   * @param str The value that need patching with zeros at the back.
   * @param len the total length of the result string.
   * @return The string patched with zeros if needed, and the length of it must have a minimum
   *     required length.
   */
  private String adaptString(String str, int len) {
    StringBuffer strbuf = new StringBuffer(str.trim());

    while (strbuf.length() < len) strbuf.insert(0, "0");
