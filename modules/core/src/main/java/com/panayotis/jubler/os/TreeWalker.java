...
public class TreeWalker {
  ...
  public static File searchExecutable(
      ArrayList<String> application, String[] parameters, String test_signature, String deflt) {
  ...
  public static File searchExecutable(
    ...
    if (root.isFile()) {
      ...
      for (String progname : program)
        if (root.getName().toLowerCase().equals(progname)
            && execIsValid(root, parameters, test_signature)) return root;
  ...
  public static boolean execIsValid(File exec, String[] parameters, String test_signature) {
    if (parameters == null || test_signature == null) return exec.isFile();

    Process proc = null;
    String[] cmd = new String[parameters.length + 1];
    cmd[0] = exec.getAbsolutePath();
    if (parameters.length > 0) System.arraycopy(parameters, 0, cmd, 1, parameters.length);

    try {
      StringBuilder buf = new StringBuilder();
      buf.append("Testing: ");
      for (int i = 0; i < cmd.length; i++) buf.append(cmd[i]).append(' ');
