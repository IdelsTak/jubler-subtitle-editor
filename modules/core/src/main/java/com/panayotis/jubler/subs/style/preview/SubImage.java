...
public class SubImage extends ArrayList<SubImage.StyledTextLine> {
  ...
  private void calculatePositions() {
    ...
    /* Calculate height */
    for (StyledTextLine line : this) height += line.height;
    /* Calculate width */
    for (StyledTextLine line : this) if (line.width > width) width = line.width;
    /* Fix edges */
  ...
  private void drawSubtitles() {
    ...
    for (int i = ((int) firstline.shadowlength - 1); i >= 0; i--)
      g.drawImage(
          ...
          (int) (i + 1),
          (int) (i + 1),
          null); // Should be reversed, for thick color matters
  ...
  private BufferedImage getOutlineImage(int txtindex, float outwidth, boolean center_draw) {
    ...
    for (StyledTextLine line : this)
      if (line != null && line.subchars != null && line.subchars[txtindex] != null)
        g.drawString(
            line.subchars[txtindex], line.dx, line.advance - line.descent); // Draw shadow mask
  ...
  class StyledTextLine {
    ...
    public void applyStyle(SubStyle style, AbstractStyleover[] overs) {
      ...
      /* Find text styles */
      applyAttributes(
          new PreviewFontname(style.get(FONTNAME), getStyleover(overs, FONTNAME)), substr);
      applyAttributes(
          new PreviewFontsize(style.get(FONTSIZE), getStyleover(overs, FONTSIZE)), substr);
      ...
      applyAttributes(
          new PreviewUnderline(style.get(UNDERLINE), getStyleover(overs, UNDERLINE)), substr);
      applyAttributes(
          new PreviewStrikethrough(style.get(STRIKETHROUGH), getStyleover(overs, STRIKETHROUGH)),
          substr);

      /* Per character colors */
      applyAttributes(
          new PreviewAlphaColor(style.get(PRIMARY), getStyleover(overs, PRIMARY)), substr[0]);
      outalpha =
          ((AlphaColor)
                  applyAttributes(
                      new PreviewColor(style.get(OUTLINE), getStyleover(overs, OUTLINE)),
                      substr[1]))
              .getAlpha();
      shadowalpha =
          ((AlphaColor)
                  applyAttributes(
                      new PreviewColor(style.get(SHADOW), getStyleover(overs, SHADOW)), substr[2]))
              .getAlpha();
      ...
      Direction dir =
          (Direction)
              applyAttributes(
                  new PreviewSingle(style.get(DIRECTION), getStyleover(overs, DIRECTION)));
      ...
      outlength =
          (Float)
              applyAttributes(
                  new PreviewSingle(style.get(BORDERSIZE), getStyleover(overs, BORDERSIZE)));
      shadowlength =
          (Float)
              applyAttributes(
                  new PreviewSingle(style.get(SHADOWSIZE), getStyleover(overs, SHADOWSIZE)));
