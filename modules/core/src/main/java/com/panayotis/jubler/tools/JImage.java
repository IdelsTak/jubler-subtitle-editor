...
public class JImage implements CommonDef {
  ...
  /** The colour that is used for DVB-T subtitle's transparency. (0x00000060) or RGB(0, 0, 96). */
  ...
  /**
   * Display an option panel which allows, among other options, browsing for directories where new
   * directory can be created. Other options include the ability to select to use current dir, the
   * ability to set the flag not to remind the creation of a new directory again in the future.
   * Setting the "Do not remind again!" will deny the ability to prompt for the option to create a
   * directory. Option "Use current" will only stops the prompt for the current set of images until
   * the maximum number of images is reached.
   *
  ...
  public static File[] createImageDirectory(File default_directory) {
    Object[] options = {__("Create/Select new"), __("Use current")};

    StringBuffer b = new StringBuffer();
    b.append(__("Current directory is\n\"{0}\"", default_directory.getAbsoluteFile()))
        .append(UNIX_NL);
    b.append(
        __(
  ...
  /**
   * Initiates the {@link #createImageDirectory} - singular - to obtain a directory selection using
   * a file dialog, defaulted at an initial directory. Once the selection has been made, the list of
   * selected directories, plus the default directory, are grouped into a single non-duplicated list
   * and returned to the calling routine.
   *
   * @param default_directory The default directory to start-up the file dialog with.
   * @return A non-duplicated list of directories which has been selected, empty if the directory
   *     selection operation has been cancelled.
   */
  public static NonDuplicatedVector<File> createImageDirectories(File default_directory) {
    ...
    boolean has_new_directories = (selectedDirectories != null) && (selectedDirectories.length > 0);
  ...
  public static boolean writeImage(
      BufferedImage img, String dir, String filename, String extension) {
    try {
  ...
  /**
   * Using the ImageIO to read an image from a file into the internal buffer, And then create a
   * image icon which is stored in the image record and update the internal image dimension if this
   * image is larger than the previous one. The image, after loaded, is cropped and transformed to a
  ...
  public static BufferedImage makeTransparentImage(
      BufferedImage img,
    ...
    try {
      ...
      for (int i = 0; i < color_index_list.length; i++) {
        ...
        if (transparentcy == 0) {
          int color_i = Integer.parseInt(color_index_list[i].toString());

          int transparent_color = Integer.parseInt(color_table[color_i].toString());
  ...
  public static BufferedImage makeTransparentImage(BufferedImage img, int color) {
    ...
    try {
      ...
      /**
       * Whilst running the loop to find boundary, also update the transparent image with desired
       * pixel, ignoring the colour that are transparent.
       */
      Rectangle sr = getSubImageDimension(img, color, tran_image);

      /** now crop the transparent image down to the size obtained. */
      sub_img = tran_image.getSubimage(sr.x, sr.y, sr.width, sr.height);

      return sub_img;
    } catch (Exception ex) {
      if (tran_image != null) return tran_image;
      else return img;
    }
  ...
  /**
   * Creates a new buffered image using TYPE_BYTE_GRAY meaning converts the current image to gray
   * scale.
   *
   * @param source The source image
   * @return The newly converted b/w image if no errors occured, source image if some errors
   *     occured.
   */
  public static BufferedImage bwConversion(BufferedImage source) {
    BufferedImage new_image = null;
  ...
  /**
   * This routine gets the sub-image boundary using the specified colour that will be treated as
   * transparent within an existing image. It assumes a transparent image 'dest' exists and write
   * the pixels that are not transparent to that image whilst searching for the boundary. This
   * routine cuts down an additional expensive loop running through the source image to find the
   * matching colour, as previously implemented using separated routines, one to find the boundary,
   * another to make the transparent image. This routine combine the 2 functions together in one
   * solution. In order to use this routine, a preparation for transparent image must be done in
   ...
   * </pre>
   *
   * The final 'sub_imag' is the cropped image and contain transparency, where all instances of
   * 'some_color' are removed (transparent).
   *
   * @param source The non-transparent image that will be used to find the sub-image boundary.
   * @param find_colour The colour that will be treated as transparent.
   * @param dest The transparent image, whose size is the same as the source image, and whose
   *     content is blank.
   * @return The dimension of sub-image where it will be cropped to, plus the destination image
   *     being filled with content that are NOT transparent.
   */
  public static Rectangle getSubImageDimension(
      BufferedImage source, int find_colour, BufferedImage dest) {
    Rectangle rec = null;
    int lx, ly, rx, ry;
    int img_w, img_h, w, h;
    try {
      ...
      /**
       * search top-left down to bottom-right and find the excluding colour. The smaller value is
       * held.
       */
  ...
  /**
   * Turns a buffered image into an image with transparency bit set for the chosen colour, making
   * the image transparent to the background or highlighting colours.
   *
   * @param source The source image to be made transparent.
   * @param find_colour The colour that will be made transparent.
   * @return The image with transparency, null if there are errors during processing.
   */
  public static BufferedImage getTransparentImage(BufferedImage source, Color find_colour) {
    ...
    try {
      ...
      for (int i = 0; i < image_data.length; i++) {
        ...
        if (!is_same) dest_image_data[i] = img_rgb; // end if
      } // end for(int i=0; i < image_data.length; i++)
  ...
  /**
   * Gets the dimension of a sub-image within an existing one by excluding a selected colour. The
   * rectangle return contains the dimension of the sub-image where the first instance of a
   * different colour to the input colour is found. This function find the boundary to be cropped.
   *
   * @param img The existing image to be cropped.
   * @param find_colour The colour where cropping will be included.
   * @return The rectangle contains the dimension where cropping can be performed.
   */
  ...
  /**
   * Note the copyright of this code is: Rafael Santos - Author of: Java Image Processing Cookbook
   * http://www.lac.inpe.br/~rafael.santos/JIPCookbook/6040-howto-compressimages.jsp
   *
   * <p>The routine compress a BufferedImage by forming a JPEG writer and write the original image
   * to the buffer of the compressed writer which will compress the image as it writes the image
   * out. The compressed image is then read back again into another instance of BufferedImage and
   * return it for use. The returned image is the compressed image of the original.
   *
   * @param image The image to be compressed.
   * @param quality The factor to compress. 1.0f being the highest quality, 0.1f being the worst.
  ...
  } // end public static BufferedImage compressImage(BufferedImage image, float quality) throws
  // IOException
  ...
  /**
   * This routine creates a blank image, by default 720x32, filled with the default {@link
   * JImage#DVBT_SUB_TRANSPARENCY transparency color}. If the input image is not empty, the input
   * image will be centered on the default image, and the height of the return image is taken from
   * the input image.
   *
   * @param center_image The input image that will be overlaid in the center of the background
   *     image.
   * @return The background image, with input image centered if any, or null if there are errors.
   */
