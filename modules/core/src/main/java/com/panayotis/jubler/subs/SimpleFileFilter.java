...
public class SimpleFileFilter extends javax.swing.filechooser.FileFilter
    implements java.io.FileFilter, CommonDef {
  ...
  public SimpleFileFilter(String description, String... extensions) {
    ...
    for (String ext : extensions)
      if (!Share.isEmpty(ext))
        lowerCaseExtensions.add(
            ext.toLowerCase(
                Locale.ENGLISH)); // end if (! Share.isEmpty(ext))//end for (String ext : exts)
  ...
  public boolean accept(File pathname) {
    if (pathname.isDirectory()) return true; // end if (pathname.isDirectory())
    String fileName = pathname.getName();
    int file_dot_pos = fileName.lastIndexOf(char_dot);
    if (file_dot_pos < 0) return false;
    ...
    for (int j = 0; j < len; j++) {
      ...
      if (ext_has_dot) ext = ext.substring(ext_dot_pos + 1); // end if (ext_has_dot)
...
} // emd public class SimpleFileFilter extends javax.swing.filechooser.FileFilter implements
  // java.io.FileFilter
