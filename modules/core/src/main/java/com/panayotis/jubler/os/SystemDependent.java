...
public class SystemDependent {
  ...
  private static void setButtonStyle(AbstractButton button, String pos, String style) {
    ...
    if (!pos.equals("only") && button.isFocusable()) button.setFocusable(false);
  }
  ...
  public static Color getWindowBackgroundColor(Component c) {
    if (IS_MACOSX) return background;
    else return c.getBackground();
  }
  ...
  public static String getKeyMods(int keymods) {
    ...
    if ((keymods & KeyEvent.META_MASK) != 0) res.append(IS_MACOSX ? "\u2318" : "+Meta");
    if ((keymods & KeyEvent.ALT_MASK) != 0) res.append(IS_MACOSX ? "\u2325" : "+Alt");
    if ((keymods & KeyEvent.CTRL_MASK) != 0) res.append(IS_MACOSX ? "\u2303" : "+Ctrl");
    if ((keymods & KeyEvent.SHIFT_MASK) != 0) res.append(IS_MACOSX ? "\u21e7" : "+Shift");
    if (res.length() > 0) res.append(' ');
    return res.toString();
  }

  public static int getDefaultKeyModifier() {
    if (IS_MACOSX) return KeyEvent.META_MASK;
  ...
  public static void appendSpotlightApplication(String name, ArrayList<ExtPath> res) {
    ...
    cmd[1] =
        "kMDItemDisplayName == '"
            + name
            + "*'"; // Use this trick to avoid spaces problems inside the filename
    try {
      String line;
      ...
      while ((line = in.readLine()) != null)
        if (line.endsWith(".app")) res.add(new ExtPath(line, ExtPath.BUNDLE_ONLY));
    } catch (Exception ex) {
    }
  }
  ...
  public static void appendLocateApplication(String name, ArrayList<ExtPath> res) {
    ...
    try {
      ...
      BufferedReader in = new BufferedReader(new InputStreamReader(proc.getInputStream()));
      while ((line = in.readLine()) != null)
        if (line.endsWith(File.separator + name)) res.add(new ExtPath(line, ExtPath.FILE_ONLY));
    } catch (Exception ex) {
    }
  }
  ...
  public static void appendPathApplication(ArrayList<ExtPath> res) {
    ...
    while (st.hasMoreTokens()) res.add(new ExtPath(st.nextToken(), 1));
  ...
  public static boolean canWrite(File f) {
    ...
    if (!IS_WINDOWS) return f.canWrite();
    ...
    if (f.isFile())
      ...
      } else ret = newfile_canwrite(f);
    else if (f.isDirectory()) ret = newfile_canwrite(new File(f, "canWrite"));
    return ret;
  }
  ...
  public static Border getBorder(String title) {
    ...
    if (border == null) border = new EtchedBorder();
    if (title == null) return border;
    else return new TitledBorder(border, title);
  }

  public static KeyStroke getUpDownKeystroke(boolean down) {
    if (IS_MACOSX)
      return KeyStroke.getKeyStroke(
          down ? KeyEvent.VK_DOWN : KeyEvent.VK_UP, InputEvent.CTRL_MASK | InputEvent.ALT_MASK);
    else
      return KeyStroke.getKeyStroke(down ? KeyEvent.VK_DOWN : KeyEvent.VK_UP, InputEvent.CTRL_MASK);
  }

  static String mapLibraryName(String name) {
    if (IS_MACOSX) return "lib" + name + ".jnilib";
    else if (IS_WINDOWS) return name + ".dll";
