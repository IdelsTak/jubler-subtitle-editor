...
public class Subtitles extends AbstractTableModel {
  ...
  public Subtitles(SubFile sfile) {
    ...
    if (sfile == null) subfile = new SubFile();
    else subfile = sfile;
  }
  ...
  private Subtitles loadByFileExtension(SubFile sfile, String data) {
    ...
    try {
      ...
      if (load != null) sfile.setFormat(format); // end if (load != null)
    } catch (Exception ex) {
    }
    return load;
  ...
  private Subtitles loadByPattern(SubFile sfile, String data) {
    ...
    try {
      ...
      if (format != null) sfile.setFormat(format); // end if (format != null)
    } catch (Exception ex) {
    }
    return load;
  } // end private Subtitles loadByFileExtension()
  ...
  public void populate(SubFile sfile, String data) {
    ...
    if (load == null) load = this.loadBySelectedHandler(sfile, data);
    if (load == null) load = this.loadByPattern(sfile, data);
    if (load != null) {
  ...
  private void appendSubs(Subtitles newsubs, boolean priority_to_new_style) {
    ...
    if (priority_to_new_style) styles.get(0).setValues(newsubs.styles.get(0));
    // TODO unused code
    SubStyle style;
    ...
    for (int i = 1; i < newsubs.styles.size(); i++) {
      ...
      if (res != 0) {
        ...
        if (priority_to_new_style) styles.get(0).setValues(newstyle);
      } else
  ...
  public void insertSubs(SubEntry location, Subtitles newsubs) {
    ...
    if (idx < 0) appendSubs(newsubs, false);
    else sublist.addAll(idx + 1, newsubs.sublist);
  }
  ...
  public SubEntry joinSubs(Subtitles s1, Subtitles s2, double dt) {
    ...
    for (int i = 0; i < s2.size(); i++) {
      ...
      add(newentry);
      if (selected == null) selected = newentry;
    }
    return selected;
  }
  ...
  public int addSorted(SubEntry sub) {
    ...
    int pos = 0;
    while (sublist.size() > pos && sublist.get(pos).getStartTime().toSeconds() < time) pos++;
    sublist.add(pos, sub);
    if (sub.getStyle() == null) sub.setStyle(styles.get(0));
    return pos;
  }
  ...
  public TotalSubMetrics getTotalMetrics() {
    ...
    for (int i = 0; i < size(); i++) max.updateToMaxValues(elementAt(i).getMetrics());
    return max;
  }
  ...
  public void revalidateStyles() {
    for (SubEntry entry : sublist) {
      ...
      if (style == null || styles.indexOf(style) < 0) entry.setStyle(styles.get(0));
    }
  }
  ...
  public void updateQuality() {
    for (SubEntry entry : sublist) entry.updateQuality();
  }
  ...
  public void setValueAt(Object value, int row, int col) {
    ...
    if (col >= FIRST_EDITABLE_COL && row < sublist.size()) sublist.get(row).setData(col, value);
  ...
  public int getColumnCount() {
    ...
    for (int i = 0; i < visiblecols.length; i++) if (visiblecols[i]) cols++;
    return cols;
  }
  ...
  /**
   * Checks to see if all entries are of text-based type, that is none of the instance of SubEntry
   * are of ImageTypeSubtitle.
   *
   * @return true if all are of text-based, false if a single instance is of different type, ie.
   *     ImageTypeSubtitle.
   */
  ...
  public boolean isRequiredToConvert(Class new_class) {
    ...
    try {
      for (SubEntry entry : sublist) {
        ...
        if (is_required) break; // end if (is_required)
      } // end for(SubEntry entry : sublist)
    } catch (Exception ex) {
    }
    return is_required;
  ...
  /**
   * This routine will convert the current subtitle-entries to a target class using the
   * implementation of 'copyRecord' method in {@link SubEntry} and its extended classes. The main
   * complex task of this is to deal with headered types. Header for one set of records is
   * considered as a global object, and hence all records in the subset inherit the same header.
   * There are situation where the target type is a headered type when the source type is not, in
   * which situation, the 'copyRecord' routine will create a new default header for a new record.
   * This reference must be kept locally and pass to all sub-sequent records, avoiding the waste of
   * memory.
   *
   * @param target_class The target class to convert the current subtitle entries to.
   * @param class_loader The classloader to use
   * @return true if convertion was carried out without erros, or no conversions are required, false
   *     otherwise.
   */
  ...
  public boolean convert(Class target_class, ClassLoader class_loader) {
    ...
    try {
      ...
      for (int i = 0; i < size(); i++) {
        ...
        if (is_same) continue; // end if (is_same)

        /** Create a new instance of the target using its class name. */
        SubEntry target_entry =
            (SubEntry) Class.forName(target_class_name, true, class_loader).newInstance();

        /**
         * Check to see if the target created is an instance of headered type. If it is, cast it to
         * the target_hdr_sub so that headering methods can be accessed.
         */
        boolean is_target_headered = (target_entry instanceof HeaderedTypeSubtitle);
        if (is_target_headered) target_hdr_sub = (HeaderedTypeSubtitle) target_entry;

        /**
         * Try to set the header record first, avoiding the copyRecord routine to create a new
         * header.
         */
        if (target_hdr_sub != null) target_hdr_sub.setHeader(header); // end if
        ...
        if (header == null && target_hdr_sub != null) header = target_hdr_sub.getHeader();
  ...
  /**
   * Moves one or more rows from the inclusive range <code>start</code> to <code>end</code> to the
   * <code>to</code> position in the model. After the move, the row that was at index <code>start
   * </code> will be at index <code>to</code>. This method will send a <code>tableChanged</code>
   ...
   * @throws ArrayIndexOutOfBoundsException if any of the elements would be moved out of the table's
   *     range
   */
