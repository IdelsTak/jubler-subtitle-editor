...
public class ReplaceModel extends AbstractTableModel {
  ...
  private static final String[][] def_replace = {
    {"\\[.*\\]", ""},
    {"@.*@", ""},
    {"\\{.*\\}", ""},
    {"<.*>", ""}
  ...
  public void setValueAt(Object value, int row, int col) {
    if (row == (replacelist.size() - 1)) replacelist.add(new ReplaceEntry());
    replacelist.get(row).setValue(col, value);
    if (col > 0) replacelist.get(row).setValue(0, true);
  }
  ...
  public final void loadOptions() {
    ...
    if (data == null || data.equals("")) {
      reset();
      return;
    } else ReplaceEntry.setData(replacelist, data);
    replacelist.add(new ReplaceEntry());
  }

  public void saveOptions() {
    StringBuilder data = new StringBuilder();
    for (int i = 0; i < (replacelist.size() - 1); i++) data.append(replacelist.get(i));
