...
public class AutoSaveOptions {
  ...
  public static void setVisibleColumns(boolean[] visiblecols) {
    ...
    for (int i = 0; i < visiblecols.length; i++) if (visiblecols[i]) out.append(COLUMNID.charAt(i));
  ...
  public static boolean[] getVisibleColumns() {
    ...
    for (int i = 0; i < COLUMNID.length(); i++)
      if (savedcols.indexOf(COLUMNID.charAt(i)) >= 0) cols[i] = true;
      else cols[i] = false;
    return cols;
  }
  ...
  public static void setColumnWidth(int[] prefcolwidth) {
    ...
    for (int i = 0; i < prefcolwidth.length; i++) widths.append(prefcolwidth[i]).append(',');
  ...
  public static int[] getColumnWidths() {
    ...
    if (widths == null || widths.equals("") || widths.length() < 1) widths = DEFAULTCOLWIDTH;
