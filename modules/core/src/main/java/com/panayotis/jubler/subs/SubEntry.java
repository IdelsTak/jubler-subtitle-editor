...
public class SubEntry implements Comparable<SubEntry>, Cloneable, CommonDef {
  ...
  static {
    ...
    styleover_template[STRIKETHROUGH.ordinal()] =
        new StyleoverCharacter(StyleConstants.StrikeThrough);
  ...
  /* Markings */
  public static final String[] MarkNames = {
    __("None"), __("Pink"), __("Yellow"), __("Cyan"), __("Orange"), __("Light Green")
  };
  ...
  /**
   * The following parameter is lazily used. It is initialized only when data are needed to be
   * added.
   */
  ...
  public void setText(String text) {
    ...
    if (overstyle != null) {
      ...
      int emptystyles = 0;
      for (int i = 0; i < overstyle.length; i++) {
        if (overstyle[i] != null) overstyle[i] = overstyle[i].setMaxStylePosition(textsize);
        if (overstyle[i] == null) emptystyles++;
      }
      if (emptystyles == overstyle.length) overstyle = null;
    }
  }
    ...
    switch (col) {
      ...
      case 6:
        return (subtext == null || subtext.length() == 0)
            ...
            : String.valueOf(
                Math.round(
                    subtext.length() * 60 / ((finish.getMillis() - start.getMillis()) / 1000d)));
      case 7:
        ...
        if (is_image_type) {
          ...
          if (has_image) {
            ...
            return img;
          } else return null; // end if (has_image)
        } else {
  ...
  void setData(int col, Object data) {
    JIDialog.error(
        null, "BUG IN PROGRAM: SET DATA WAS SELECTED\nPlease contact author", __("Error!"));
  ...
  public void cleanupEvents() {
    ...
    if (overstyle[i] != null) overstyle[i].cleanupEvents(style.get(i), subtext);
  }
  ...
  public void applyAttributesToDocument(JTextPane pane) {
    ...
    for (int i = 0; i < styleover_template.length; i++)
      if (overstyle == null || overstyle[i] == null)
        StyleoverCharacter.applyAttributesToDocument(
            pane.getStyledDocument(), style.get(i), styleover_template[i], subtext.length());
      else
        overstyle[i].applyAttributesToDocument(
            pane.getStyledDocument(), style.get(i), subtext.length());
  }

  public void insertText(int start, int length) {
    for (int i = 0; i < styleover_template.length; i++)
      if (overstyle != null && overstyle[i] != null) overstyle[i].insertText(start, length);
  }
  ...
  public void updateQuality(SubMetrics m) {
    if (m.lines > getMaxLines()
        || m.cps > getMaxCPS()
        || m.length > getMaxSubLength()
        || m.linelength > getMaxLineLength()
        || finish.differenceInSecs(start) > getMaxDuration()
        || finish.differenceInSecs(start) < getMinDuration()
  ...
  /**
   * Duration of a subtitle entry sometimes could be considered too small - by errors or any other
   * reasons and in such a case it should be notified to the editor.
   *
   * @return true if the value for duration exists and the length is less than 500 milliseconds,
   *     false otherwise.
   */
  ...
  public Object clone() {
    ...
    try {
      ...
      if (start != null) new_sub.start = new Time(start);
      if (finish != null) new_sub.finish = new Time(finish);
      if (subtext != null) new_sub.subtext = new String(subtext);
  ...
  /**
   * Performs a deep-copy of other record's content. This is necessary when perform import/export
  ...
  public void copyRecord(SubEntry o) {
    try {
      if (o.start != null) start = new Time(o.start);
      if (o.finish != null) finish = new Time(o.finish);
      if (o.subtext != null) subtext = new String(o.getText());
      setMark(o.getMark());
      style = o.style;
      if (o.overstyle != null) {
  ...
  /**
   * Merge two records together by appending the other record's text onto this instance. The
   * starting time will be the earlier one of the two, and so the ending will be the later one of
   * the two. It is expected that the other record, after the merge, be removed to avoid
   * duplication.
   *
  ...
  /**
   * This function collect words from a list and joining them with a space character.
   *
  ...
  private String collectWord(String[] list, int from_idx, int to_idx) {
    ...
    for (int i = from_idx; i < to_idx; i++) {
      ...
      if (!(is_last_word || isHyphenatedWord(word))) b.append(char_sp); // end if (! is_last_word)
    } // end for
    return b.toString();
  }

  /**
   * Split the current record into two instances. Steps involved: 1. Cloning the current instance to
   * enable details such as attributes to be copied to the second instance. 2. Subtitle text of this
   * instance is converted into a list of words, space separated. The list is halved between two
   * instances. This instance takes the first half, the copy takes the second half. 3. The timing
   * duration is halved. This instance takes the first half, the new instance takes the second half.
   *
   * @return The clone instance of the current entry, with halves of the subtitle text and time. It
   *     is meant that the return instance be placed after the current instance in the subtitle's
   *     time-line.
   */
  public SubEntry splitRecord() {
    ...
    try {
      ...
      if (ok) {
        ...
        new_sub.setText(s2);
      } else new_sub.setText(new String()); // end if
  ...
  /**
   * Gets the number of text lines by split the text line at the the new-line characters and returns
  ...
  /**
   * Convert the subtitle text into a list of lines, separating lines at the new-line character -
   * '\n' - and collate them into a vector of strings. This is convenient for operations that deals
   * with text lines in its congruent order.
   *
   * @return A vector containing all text lines of the subtitle text. An empty list if no text is
   *     available.
   */
   ...
   * @return true if the operation has been carried out without errors, false otherwise.
   */
  public boolean setText(Vector<String> list) {
    try {
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (!is_last_line) b.append(UNIX_NL); // end if (! is_last_line)
  ...
  /**
   * This routine removes line breaks in the subtitle text and return the text with a string of
   * words, each separated by a single space.
   *
   * @return The subtitle text of this subtitle event as a continous string of words, each separated
   *     by a single space. All new-lines are removed.
   */
   ...
   * @return the text line at the designated index, null if the text line at the required index
   *     doesn't exists.
   */
  ...
  /**
   * Get the last text line of the subtitle text, such as line at index = length-1.
   *
  ...
  /**
   * Cheks to see if the text line, at the same index, on current subtitle event and an other event,
   ...
   * @return true if the text lines are identical in content, case sensitive, false otherwise.
   */
  ...
  /**
   * Cheks to see if the text line, at the first index, on current subtitle event and an other
   * event, are identical or not.
   *
   * @param other The other subtitle event to be compared.
   * @return true if the text lines are identical in content, case sensitive, false otherwise.
   */
  ...
  /**
   * Cheks to see if the text line at the last index, on current subtitle event, and one on the
   * first index on an other event, are identical or not. This check serves the text duplication
   * tests when performing duplication removals.
   *
   * @param other The other subtitle event to be compared.
   * @return true if the text lines are identical in content, case sensitive, false otherwise.
   */
  ...
  /**
   * Get the text line excluding a selected line. This routine serves the removal of the duplicated
   * text lines. The index is zero-based.
   *
  ...
  /**
   * Remove the top line of the subtitle text and returns the remaining lines in a vector of
   * strings.
   *
  ...
  /**
   * Remove the text line at a chosen index by converting the subtitle text into a vector of string,
   * remove the text line at the desired index, then reset the current subtitle text with the
   * remaining lines in the list.
   *
  ...
  /**
   * Examines to see if the starting-time of this subtitle-event and other's are the same or not.
   *
   * @param o The other instance of subtitle event, whose starting-time will be compared to the this
   *     instance.
   * @return true if the starting-times are identical or their difference is considered to be too
  ...
  /**
   * Examines to see if the ending-time of this subtitle-event and other's are the same or not.
   *
   * @param o The other instance of subtitle event, whose end-time will be compared to the this
   *     instance.
   * @return true if the ending-times are identical or their difference is considered to be too
   *     small. False otherwise.
   * @see #SMALL_MILLIS
   */
  ...
  /**
   * Add a string instance to the current subtitle text, using a selective separator.
   *
   * @param line The new string instance to be added to the current text. This could be a single
   *     word, or a new multi-word text-line.
   * @param separator The chosen separator for the current text and the new instance. This could be
   *     a space for word or a new-line for a line.
   * @return true if the new text-line has been added to cthe current text, false otherwise.
   */
  ...
  /**
   * Similar to the previous append text, but this function checks to see if the new text is single
   * word or not. A single word is added with space separator, where multi-words lines are added
   * with new-line character seperations.
   *
   * @param text_list The vector containing the group of text lines to be added.
  ...
  public boolean appendText(Vector<String> text_list) {
    try {
      ...
      if (is_one_line) {
        ...
        int word_count = Share.wordCount(text);
        boolean is_one_word = (word_count == 1);
        if (is_one_word) addWord(text);
        else addTextLine(text); // end if (is_one_word)
      } else addAllText(text_list); // end if (is_one_line)
      return true;
    } catch (Exception ex) {
      return false;
    }
  ...
  /**
   * Appending a new text line to the current subtitle text lines, separating them with a '\n'
   * character.
   *
   * @param line The new text line to be appended to the current text.
   * @return true if the line has been appended without errors, false otherwise.
   */
  ...
  /**
   * Adds a single word into the current subtitle text, using a single space character as separator.
   *
  ...
  /**
   * There are times where text spacing causes the text comparison between two instances of subtitle
   * events, when in fact, their contents are identical. To eleminate this differences, this routine
   * respaces words and makes sure that they are single-spaced.
   *
   * @return true if the changes has been carried out without errors, false otherwise.
   */
  public boolean reSpacingText() {
    try {
      ...
      for (int i = 0; i < len; i++) {
        ...
        boolean is_last_line = (i == len - 1);
        if (!(is_last_line || isHyphenatedWord(word))) b.append(char_sp); // end if (! is_last_line)
  ...
  /**
   * Remove line break at a specific line, by replacing the new-line with a space character. The
   * routine reconstruct the subtile text by first convert the text into a vector of strings, then
   * using the string buffer, appending each line back into the buffer, replacing the line separator
   * by a space if the line is required line, else replaced by a new-line, before subsequent lines
   * are appended into the buffer.
   *
   * @param line_number The line number at which, the end of line character is being replaced by a
   *     space before the next line is appended on.
  ...
  public boolean copyHeader(SubEntry source) throws Exception {
    ...
    if (!is_same_type)
      throw new IncompatibleRecordTypeException(
          hdr_current_entry.getClass(), hdr_import_entry.getClass()); // end if (! is_same_type)
  ...
  /**
   * This routine made use of {@link SubImage} to draw the text image. When creating the image, all
   * the text attributes such as font, size, outline, shadow etc.. will be taken into account.
   *
   * @return The image of the text, or null if there are errors during the image creation.
   */
