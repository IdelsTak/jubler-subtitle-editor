...
public class Options {
  ...
  private static final Properties opts;
  private static final String preffile;
  private static final Preferences prefs = Preferences.userNodeForPackage(Options.class);
  ...
  private static String updateConfigFile() {
    ...
    if (oldconfig.exists())
      if (!newconfig.exists()) {
        ...
        if (!success) DEBUG.debug("Unable to move configuration file to " + newconfig.getPath());
        else DEBUG.debug("Configuration file moved to " + newconfig.getPath());
      }
  ...
  public static void loadSystemPreferences(JPreferences prefs) {
    for (TabPage opt : prefs.Tabs.getTabArray()) ((OptionsHolder) opt).loadPreferences();
  }

  public static void saveSystemPreferences(JPreferences prefs) {
    for (TabPage opt : prefs.Tabs.getTabArray()) ((OptionsHolder) opt).savePreferences();
    saveOptions();
  }
  ...
  public static void saveFileList(Stack<SubFile> recents) {
    ...
    while (counter < MAX_RECENTS) opts.remove("System.Lastfile" + (++counter));
    saveOptions();
  }
  ...
  public static Stack<SubFile> loadFileList() {
    ...
    for (int i = MAX_RECENTS; i > 0; i--)
      try {
        ...
        f = sf.getSaveFile();
        if (f.exists() && f.canRead() && f.isFile()) files.push(sf);
