...
public class JFramePreview extends JPanel {
  ...
  private static final String inactive_decoder_message =
      __("FFDecode library not active. Using demo image.");
  private final Image demoimg;
  /* Maximum amount of time tolerance while requesting a new image */
  public static final double DT = 0.002d;
  ...
  public void repaint() {
    ...
    if (isEnabled()) {
      ...
      if (Math.abs(time - last_time) > DT || frameimg == demoimg) {
        last_time = time;
        if (mfile != null) newimg = mfile.getFrame(last_time, resize);
        else newimg = null;
      }
      if (newimg != null) frameimg = newimg;
    } else frameimg = demoimg;
    super.repaint();
  }
  ...
  public void paintComponent(Graphics g) {
    ...
    if (getWidth() > imgwidth) g.fillRect(imgwidth, 0, getWidth() - imgwidth, getHeight());
    ...
    g.drawImage(
        ...
        0,
        REEL_OFFSET,
        null); // Since we have already loaded the picture from memory, the imageobserver is of no
    // help
    if (subimg != null)
      g.drawImage(
          subimg.getImage(),
          subimg.getXOffset(frameimg),
          subimg.getYOffset(frameimg) + REEL_OFFSET,
          (ImageObserver) null);
    ...
    if (!mfile.getDecoder().isDecoderValid()) {
      ...
      g.setFont(f);
      TextLayout layout =
          new TextLayout(inactive_decoder_message, f, ((Graphics2D) g).getFontRenderContext());
      g.setColor(Color.RED);
      g.fillRect(
          2,
          2 + REEL_OFFSET,
          (int) layout.getAdvance() + 1,
          (int) layout.getAscent() + (int) layout.getDescent() + 1);
      g.setColor(Color.WHITE);
