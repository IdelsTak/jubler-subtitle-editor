...
public class LoaderThread extends Thread {
    ...
    while (true)
      try {
        ...
        while (loader.sublist.size() > 0) {
          ...
          File f = new File(sub);
          if (f.getName().startsWith(AutoSaver.AUTOSAVEPREFIX)) autosave_counter++;
        ...
        synchronized (loader.sublist) {
          // AutoSaver SHOULD be called after the initial loading has been performed, or else
          // auto-saved files will vanish!
          AutoSaver.launch();
          try {
            loader.sublist.wait();
  ...
  public void addSubList(String[] list) {
    for (String item : list) addSubtitle(item);
  }

  public void sendToMaster() {}

  public void goToMaster() {
    for (String item : loader.sublist) JublerClient.setFileList(item);
    System.exit(0);
  }
}
