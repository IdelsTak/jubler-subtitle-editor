...
public abstract class NativeDecoder implements DecoderInterface {
  ...
  public boolean initAudioCache(AudioFile afile, CacheFile cfile, DecoderListener fback) {
    ...
    if (cacher != null) {
      JIDialog.error(
          null,
          __("Still creating cache. Use the Cancel button to abort."),
          __("Caching still in progress"));
      return false;
    }
    if (afile == null) {
    ...
    cacher =
        new Thread() {
          public void run() {
            ...
            cacher =
                null; // Needed early, to "tip" the system that cache creating has been finished
            setInterruptStatus(false);

            if (!status)
              JIDialog.error(
                  null,
                  __("Error while loading file {0}", af.getPath()),
  ...
  public void closeAudioCache(CacheFile cfile) {
    ...
    if (cacher != null) setInterruptStatus(true);

    /* Now clean up memory */
    if (cfile != null && isDecoderValid()) forgetCache(cfile.getPath());
  }
