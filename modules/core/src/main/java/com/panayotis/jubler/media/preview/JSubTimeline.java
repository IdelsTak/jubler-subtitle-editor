...
public class JSubTimeline extends JPanel {
  ...
  private int findAction() {
    ...
    SubInfo infback =
        last_selected_subinfo; // Store this variable in case visual list is not THAT important
    vaction = checkAListForAction(vislist); // Check visual list what action it is there
    if ((vaction & RESIZE)
        != 0) { // Take into account the visual list ONLY if the selected action is Resize (which is
      // very strong)
  ...
  /**
   * Check if a subinfo is inside this list AND it's position is relative to the mouse cursor (so we
   * need to take action like moving or resizing
   */
  ...
  public void mouseStartsDragging(MouseEvent e) {
    ...
    last_subinfo_was_selected =
        (selection_mode & SELECTED_GROUP) > 0; // Check if last selected sub is already selected
    ...
    if (selection_mode == RESIZERIGHT) {
      ...
      for (SubInfo inf : sellist) if (central_point > inf.start) central_point = inf.start;
    } else {
      central_point = Double.MIN_VALUE;
      for (SubInfo inf : sellist) if (central_point < inf.end) central_point = inf.end;
    }
  ...
  public void mouseStillDragging(MouseEvent e) {
    ...
    if (selection_mode == MOVE) {
      ...
      double factor =
          (e.getX() - central_point * getWidth())
              / ((double) X_position - central_point * getWidth());
  ...
  public void mouseStopsDragging(MouseEvent e) {
    ...
    /**
     * Use this method to clear the selection list - if necessary. These actions are performed ONLY
     * when a selection has been performed and NOT when a dragging has been done (eg. resize/move).
    ...
    if (last_selected_subinfo
        == null) { // A move has been performed, the subtitles have been changed
      ...
      else /* or else, keep it in place */ offset_time = 0;
      ...
      for (int i = 0; i < sellist.size(); i++) {
        ...
        /* Set new times */
        entry.setStartTime(
            new Time(view.getStart() + view.getDuration() * info.start + offset_time));
        entry.setFinishTime(
            new Time(view.getStart() + view.getDuration() * info.end + offset_time));
      }
    }
  ...
  /**
   * Use this functiopn when a change has been performed in the selected subtitles. No changes to
   * the displayed subtitles will be performed
   */
  private void selectionHasChanged(int[] subid) {
    ...
    if (subid == null) {
      ...
      for (int i = 0; i < subid.length; i++) subid[i] = sellist.get(i).pos;
    }
    ...
    for (int i = 0; i < subid.length; i++) {
      ...
      if ((where = findInList(vislist, subid[i])) >= 0) vislist.remove(where);
    }
  }
  ...
  /**
   * Use this methid to inform that the visible window has been changed AND/OR the selection has
   * been changed
   */
  ...
  private int findInList(ArrayList<SubInfo> list, int i) {
    for (int j = 0; j < list.size(); j++) if (list.get(j).pos == i) return j;
    return -1;
  }
  ...
  public double getCenterOfSelection() {
    if (sellist.size() == 0) return view.getStart() + view.getDuration() / 2d;
  ...
  public void paintComponent(Graphics g) {
    ...
    for (int i = 0; i < vislist.size(); i++) {
      inf = vislist.get(i);
      g.fill3DRect(
          (int) (inf.start * width), 0, (int) ((inf.end - inf.start) * width), height, true);
    }
