...
public class ExternalTool {
  ...
  public void exec(final JubFrame jubler) {
    try {
      ...
      String input =
          new File(tempDir, "input" + (inplace ? "output" : "") + ".srt").getAbsolutePath();
      ...
      final JExternalConsole console =
          new JExternalConsole(
              jubler,
              new Runnable() {
                @Override
                public void run() {
...
class ProcThread extends Thread {
  ...
  public void run() {
    try (BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(asError ? proc.getErrorStream() : proc.getInputStream()))) {
      String line;
      while ((line = reader.readLine()) != null) {
        if (asError) console.addErrLine(line);
        else console.addOutLine(line);
