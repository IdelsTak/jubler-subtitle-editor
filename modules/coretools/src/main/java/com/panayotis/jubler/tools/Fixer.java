...
public class Fixer extends OneByOneTool {
  ...
  protected void storeSelections() {
    ...
    if (vis.SortB.isSelected())
      subtitles.sort(
          ((JTimeRegion) getTimeArea()).getStartTime(),
          ((JTimeRegion) getTimeArea()).getFinishTime());

    /* What to do with the remaining duration */
    fix = vis.FixT.isSelected();
    if (fix) pushmodel = vis.PushModelB.getSelectedIndex();
  ...
  protected void affect(SubEntry sub) {
    ...
    if (mindur < 0 && maxdur < 0) mindur = maxdur = curdur;
    else if (mindur < 0) mindur = (curdur > maxdur) ? maxdur : curdur;
    else if (maxdur < 0) maxdur = (curdur < mindur) ? mindur : curdur;
    ...
    switch (pushmodel) {
      ...
      default:
        ...
        if ((curdur + 2 * gap) <= avail) {
          ...
          double center =
              curstart + (curdur / 2); /* Calculate the old center of the subtitle display */
          double newstart =
              center - (fulldur / 2); /* Calculate the new *full* start of the subtitle */
          double newfinish =
              center + (fulldur / 2); /* Calculate the new *full* finish of the subtitle */
          ...
          if (prev != null)
            newbegin +=
                (gap
                    * factor); /* Add the gap only if it's between subtitles (to fix an error with the first subtitle)
