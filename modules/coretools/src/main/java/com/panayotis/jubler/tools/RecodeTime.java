...
public class RecodeTime extends RealTimeTool {
  ...
  public boolean setValues(TimeSync first, TimeSync second) {
    ...
    given_center =
        (t2.timediff * t1.timepos - t1.timediff * t2.timepos) / (t2.timediff - t1.timediff);
    ...
    given_factor =
        (t1.timepos - t2.timepos + t1.timediff - t2.timediff) / (t1.timepos - t2.timepos);
  ...
  public void storeSelections() {
    ...
    RecodeTimeGUI vis = (RecodeTimeGUI) getToolVisuals();
    try {
      if (vis.AutoB.isSelected()) factor = vis.FromR.getFPSValue() / vis.ToR.getFPSValue();
      else factor = Double.parseDouble(vis.CustomF.getText());
