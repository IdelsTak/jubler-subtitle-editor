...
public class W3CTimedText extends AbstractTextSubFormat {
  ...
  protected SubEntry getSubEntry(Matcher m) {
    ...
    SubEntry entry =
        new SubEntry(start, finish, m.group(10).replace("&amp;", "&").replace("<br />", "\n"));
    return entry;
  }
  ...
  protected void appendSubEntry(SubEntry sub, StringBuilder str) {
    str.append("      <p begin=\"")
        .append(sub.getStartTime().getSeconds().replace(',', '.'))
        .append("\" end=\"")
        .append(sub.getFinishTime().getSeconds().replace(',', '.'))
        .append("\">");
