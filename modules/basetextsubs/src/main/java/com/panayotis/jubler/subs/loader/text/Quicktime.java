...
public class Quicktime extends AbstractTextSubFormat {
  ...
  static {
    pat = Pattern.compile("(?s)\\[(\\d\\d):(\\d\\d):(\\d\\d)\\.(\\d+)\\](.*?)((?=\\[)|\\z)");
  ...
  protected void initSaver(Subtitles subs, MediaFile media, StringBuilder header) {
    ...
    start = null;
    finish = null;
    if (media.getVideoFile() != null) mediafinish = new Time(media.getVideoFile().getLength());
  ...
  protected void appendSubEntry(SubEntry sub, StringBuilder str) {
    ...
    if (start.compareTo(finish) > 0) printTime(str, start);
    /* Print subtitle */
