...
public class SubStationAlpha extends StyledTextSubFormat {
  ...
  static {
    pat =
        Pattern.compile(
            ...
            "(?i)Dialogue:(.*?),(\\d):(\\d\\d):(\\d\\d)\\.(\\d\\d),(\\d):(\\d\\d):(\\d\\d)\\.(\\d\\d),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?)"
                + nl);

    testpat =
        Pattern.compile(
            "(?i)(?s)\\[Script Info\\].*?\\[V4 Styles\\].*?"
                + "Dialogue:.*?,.*?,.*?,.*?,.*?,.*?,.*?,.*?,.*?,.*?"
                + nl);
    ...
    styles_dict.add(
        new StyledFormat(
            UNKNOWN, "",
            null)); // Add this line if you want this style to save unknwn formats. It has to be
    // LAST since it matches ALL tags
  }
  ...
  protected SubEntry getSubEntry(Matcher m) {
    ...
    SubEntry entry =
        new SubEntry(start, finish, m.group(16).replace("\\N", "\n").replace("\\n", "\n"));
  ...
  protected void appendSubEntry(SubEntry sub, StringBuilder str) {
    ...
    if (sub.getStyle() == null) str.append("*Default");
    else {
      if (sub.getStyle().isDefault()) str.append('*');
      str.append(sub.getStyle().Name);
    }
    str.append(",").append(sub.getName()).append(',');
    str.append(sub.getMarginL())
        .append(',')
        .append(sub.getMarginR())
        .append(',')
        .append(sub.getMarginV())
        .append(',');
